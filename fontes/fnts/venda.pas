...
var
  ...
  cod_apoio: integer;
  bLanca_pre_venda : Boolean;
  VendaFechamento : boolean;
  observ: string;
...
uses modulo, venda_fechamento, venda_contasreceber,
  ...
  venda_devolucao, orcamento_localizar, OpNatureza, pre_venda,
  venda_fechamento2;
...
begin
  ...
  if frmmodulo.qrconfig.fieldbyname('Venda_fechamento_avulso').AsInteger = 1 then
    VendaFechamento := true
  else
    VendaFechamento := false;
      ...
      if rtotal.Value <= 0 then
       ...
       begin
         if VendaFechamento then
           ...
           begin
            frmvenda_fechamento2 := tfrmvenda_fechamento2.create(self);
            frmvenda_fechamento2.Tag := 100;
            frmvenda_fechamento2.showmodal;
           end;
  ...
  if finalizou then
   begin
    frmPre_Venda := tfrmPre_venda.create(self);
     with  frmPre_Venda do
     begin
      qrPre_Venda.CLOSE;
      qrPre_Venda.SQL.CLEAR;
      qrPre_Venda.SQL.ADD('select * from c000074 where codigo = ' + quotedstr(sCodNota));
      qrPre_Venda.open;
     //  showmessage(qrpre_venda.sql.text);
      if qrpre_venda.recordcount > 0 then
       begin
        qrPre_Venda.edit;
        qrPre_Venda.fieldbyname('situacao').asinteger := 2;
        qrPre_Venda.post;
       end;
...
procedure Tfrmvenda.P1Click(Sender: TObject);
begin
