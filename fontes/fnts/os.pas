...
type
  Tfrmos = class(TForm)
    ...
    ComboVendedor: TwwDBLookupComboDlg;
    qros_produtosVENDEDOR: TStringField;
    PageSheet8: TPageSheet;
    dbgrd1: TDBGrid;
    ds1: TDataSource;
    fltfld_servicoDESC_PARCIAL: TFloatField;
...
uses modulo, loc_funci, principal, loc_cliente, loc_setor,
  ...
  os_contasreceber, xloc_cliente, os_servicosperiodicos, os_item_2, Os_Analise;
...
begin
  ...
  qrcontasreceber.SQL.add('select a.codigo as  Documento,'+
                                'fun.nome vendedor,'+
                                'a.codcaixa  caixa,'+
                                'a.data_emissao  emissao,'+
                                'a.data_vencimento  Vencimento,'+
                                'a.data_pagamento  Pagamento,'+
                                'a.valor_original  Valor,'+
                                'a.valor_pago  Pago,'+
                                'a.tipo as forma '+
                                'from c000049 a,c000008 fun '+
                                'where  a.codvendedor = fun.codigo and a.codigo like ''%'+
                          DBText1.Caption +'%'''+' and a.codcliente = '+ quotedstr(ecliente.text));
  qrcontasreceber.Open;
  qrcontasreceber.Fields[1].DisplayWidth := 40;
  qrcontasreceber.IndexFieldNames := 'Documento';
...
procedure Tfrmos.N3Click(Sender: TObject);
begin
  try
  FrmOsAnalise := TFrmOsAnalise.Create(self);
  FrmOsAnalise.showmodal;
  finally
  FrmOsAnalise.free;
  end;
end;
...
var
OsTotal,OsSubTotal,OsDesconto : real;
begin
   if  (DBEdit6.Text <> '')  and (DBEdit6.Text > '0') then
    begin
       if application.messagebox('O Valor do Desconto sera distribuido pela quantidade de serviços ', 'Atenção!', mb_yesno + MB_ICONWARNING ) = idyes  then
         begin
          tedit(sender).color := clwindow;
          OsSubTotal := frmmodulo.cdsos.fieldbyname('servico_subtotal').asfloat;
          OsDesconto := frmmodulo.cdsos.fieldbyname('servico_desconto').asfloat;
      //    OsTotal := OsSubTotal - OsDesconto;
          OsTotal := OsDesconto / qros_servico.RecordCount;
          frmmodulo.cdsos.fieldbyname('servico_total').asfloat := OsTotal ;
          qros_servico.first;
          while not qros_servico.eof do
           begin
            qros_servico.edit;
            QROS_SERVICO.fieldbyname('DESC_PARCIAL').asfloat := osTotal ;
            qros_servico.post;
            qros_servico.next;
           end
         end
...
procedure Tfrmos.ComboVendedorEnter(Sender: TObject);
begin

 //if frmmodulo.cdsos.State = dsedit then  qros_produto.edit;
end;

procedure Tfrmos.ComboVendedorExit(Sender: TObject);
begin
  frmmodulo.qrfunci.close;
  frmmodulo.qrfunci.sql.clear;
 ...
 if combovendedor.Text <> '' then
  begin
    qros_produtos.Edit;
    qros_produtos.fieldbyname('codvendedor').asstring := frmprincipal.zerarcodigo(combovendedor.Text, 6);
   if frmmodulo.qrfunci.Locate('codigo', ComboVendedor.Text, [loCaseInsensitive]) then
    begin
      //
    end
    else
    begin
      ...
      qros_servico.fieldbyname('codvendedor').asstring := '';
     // combovendedor.setfocus;
    end;
   showmessage(qros_produtos.fieldbyname('codigo').asstring);
   qros_produtos.post;
   //FRMMODULO.cdsos.edit;
    {
   qros_produtos.close;
  qros_produtos.sql.clear;
  qros_produtos.sql.add('select * from c000032');
  qros_produtos.open;
     }
  end;
...
begin
  ...
  frmmodulo.cdsos.fieldbyname('produto_total').asfloat := frmmodulo.cdsos.fieldbyname('produto_subtotal').asfloat -
