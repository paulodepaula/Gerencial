...
uses modulo, principal,
  loc_cliente, produto_pedido_item, Math, email, produto_pedido,
  produto_pedidoVenda_item;

{$R *.dfm}

procedure Tfrmproduto_Venda.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  if (frmmodulo.QrPedido_venda.State = dsinsert) or
    (frmmodulo.QrPedido_venda.State = dsedit) then
    bcancelarClick(frmproduto_pedido);
  qrpedido_item.close;
  Action := cafree;
end;

procedure Tfrmproduto_Venda.FormShow(Sender: TObject);
begin
  // panel_item.top := 375;
  // panel_item.left := 2;
  ...
  frmmodulo.qrProduto_Venda.close;
  frmmodulo.qrProduto_Venda.sql.clear;
  frmmodulo.qrProduto_Venda.sql.add('select * from c000141 order by codigo');
  frmmodulo.qrProduto_Venda.open;
  frmmodulo.qrProduto_Venda.last;
 ...
 // frmmodulo.qrProduto_VendaTOTAL_NOTA.DisplayFormat:= mascara_valor;
  //frmmodulo.qrProduto_VendaToTal
 // frmmodulo.qrProduto_Venda.FieldByName('TOTAL_NOTA').AsString := mascara_valor;
end;

procedure Tfrmproduto_Venda.enomeEnter(Sender: TObject);
begin
  TEdit(Sender).Color := $00A0FAF8;
end;

procedure Tfrmproduto_Venda.enomeExit(Sender: TObject);
begin
  TEdit(Sender).Color := clwindow;
end;

procedure Tfrmproduto_Venda.bincluirClick(Sender: TObject);
begin
  frmmodulo.QrPedido_venda.open;
  frmmodulo.QrPedido_venda.insert;
  frmmodulo.QrPedido_venda.fieldbyname('codigo').asstring :=  frmprincipal.codifica('000141','update');
  frmmodulo.QrPedido_venda.fieldbyname('numero').asstring :=  frmmodulo.QrPedido_venda.fieldbyname('codigo').asstring;
  frmmodulo.QrPedido_venda.fieldbyname('situacao').asstring := '1';
...
procedure Tfrmproduto_Venda.AdvMetroButton1Click(Sender: TObject);
begin
  close;
end;

procedure Tfrmproduto_Venda.balterarClick(Sender: TObject);
begin
...
procedure Tfrmproduto_Venda.bexcluirClick(Sender: TObject);
begin
  if frmprincipal.autentica('Excluir Pedido', 4) then
  begin
    qrgrade_pedido.close;
    qrgrade_pedido.sql.clear;
    qrgrade_pedido.sql.add('delete from c000143 where codnota = ''' +
      frmmodulo.QrPedido_venda.fieldbyname('codigo').asstring + '''');
    qrgrade_pedido.ExecSQL;
    frmprincipal.logUC('Excluiu Pedido Compra - ' + DBEdit2.Text, 3);
    frmmodulo.QrPedido_venda.Delete;
...
procedure Tfrmproduto_Venda.blocalizarClick(Sender: TObject);
begin
...
procedure Tfrmproduto_Venda.bfecharClick(Sender: TObject);
begin
  close;
end;

procedure Tfrmproduto_Venda.bgravarClick(Sender: TObject);
var
...
begin
  ...
  begin
    if (frmmodulo.QrPedido_venda.State = dsinsert) then
    begin
      frmprincipal.logUC('Incluiu Pedido Compra - ' + DBEdit2.Text, 1);
      frmmodulo.QrPedido_venda.post;
    end;

    if (frmmodulo.QrPedido_venda.State = dsedit) then
    begin
...
procedure Tfrmproduto_Venda.bcancelarClick(Sender: TObject);
begin
  ...
  if (frmmodulo.QrPedido_venda.State = dsinsert) or
    (frmmodulo.QrPedido_venda.State = dsedit) then
    frmmodulo.QrPedido_venda.cancel;

  frmmodulo.Conexao.Rollback;

  frmmodulo.QrPedido_venda.Refresh;
...
procedure Tfrmproduto_Venda.enomeKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    PERFORM(WM_NEXTDLGCTL, 0, 0);
end;

procedure Tfrmproduto_Venda.rnomeKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    LOC.setfocus
end;

procedure Tfrmproduto_Venda.rcodigoKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    LOC.setfocus;
end;

procedure Tfrmproduto_Venda.LOCKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    bloc.setfocus;
end;

procedure Tfrmproduto_Venda.BLOCClick(Sender: TObject);
begin
  qrpedido_produto.close;
  qrpedido_produto.sql.clear;
  if rnome.Checked then
  begin
    qrpedido_produto.sql.add('select * from c000141 where upper(numero) = ''' +
  ...
  begin
    qrpedido_produto.sql.add
      ('select * from c000141 where data = :datai order by data');
    qrpedido_produto.Params.ParamByName('datai').value := strtodate(LOC.Text);
  end;
  if rapelido.Checked then
  begin
    qrpedido_produto.sql.add('select * from c000141 where codfornecedor = ''' +
...
procedure Tfrmproduto_Venda.DBGrid1KeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
    try
      frmmodulo.QrPedido_venda.Locate('codigo',
...
procedure Tfrmproduto_Venda.E1Click(Sender: TObject);
var
  Para: string;
  bserv: boolean;
...
procedure Tfrmproduto_Venda.etipoExit(Sender: TObject);
begin
  TEdit(Sender).Color := clwindow;
end;

procedure Tfrmproduto_Venda.edata_cadastroEnter(Sender: TObject);
begin
  TEdit(Sender).Color := $00A0FAF8;
  if frmmodulo.QrPedido_venda.State = dsinsert then
...
procedure Tfrmproduto_Venda.elimiteKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    bgravar.setfocus;
end;

procedure Tfrmproduto_Venda.DBEdit1Enter(Sender: TObject);
begin
  TEdit(Sender).Color := $00A0FAF8;
end;

procedure Tfrmproduto_Venda.DBEdit1Exit(Sender: TObject);
begin
  TEdit(Sender).Color := clwindow;

end;

procedure Tfrmproduto_Venda.DBEdit1KeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    PERFORM(WM_NEXTDLGCTL, 0, 0);
end;

procedure Tfrmproduto_Venda.RxDBCalcEdit2KeyPress(Sender: TObject;
  var Key: Char);
begin
  if Key = #13 then
    bgravar.setfocus;
...
procedure Tfrmproduto_Venda.FecharLocalizao1Click(Sender: TObject);
begin
  PopupMenu := Pop1;
  qrpedido_produto.close;
  plocalizar1.visible := false;
...
procedure Tfrmproduto_Venda.edata_cadastroExit(Sender: TObject);
begin
  TEdit(Sender).Color := clwindow;
...
procedure Tfrmproduto_Venda.bfinalizarClick(Sender: TObject);
begin
  bgravar.setfocus;
end;

procedure Tfrmproduto_Venda.bexcluir_itemClick(Sender: TObject);
begin
  ...
  begin
    qrgrade_pedido.close;
    qrgrade_pedido.sql.clear;
    qrgrade_pedido.sql.add('delete from C000143 where coditem = ''' +
      dspedido_item.DataSet.fieldbyname('codigo').asstring + '''');
    qrgrade_pedido.ExecSQL;
    frmmodulo.QrPedido_venda.fieldbyname('total_nota').asfloat :=
      frmmodulo.QrPedido_venda.fieldbyname('total_nota').asfloat -
      dspedido_item.DataSet.fieldbyname('TOTAL').asfloat;

    dspedido_item.DataSet.Delete;

    frmmodulo.QrPedido_venda.fieldbyname('itens').asinteger :=
      frmmodulo.QrPedido_venda.fieldbyname('itens').asinteger - 1;
    grid1.setfocus;
  end
  else
  begin
...
procedure Tfrmproduto_Venda.DBEdit2Exit(Sender: TObject);
begin
  if (frmmodulo.QrPedido_venda.State = dsinsert) or
    (frmmodulo.QrPedido_venda.State = dsedit) then
    frmmodulo.QrPedido_venda.fieldbyname('numero').asstring :=
      frmprincipal.zerarcodigo(DBEdit2.Text, 6);
  TEdit(Sender).Color := clwindow;
end;

procedure Tfrmproduto_Venda.blocfornecedorClick(Sender: TObject);
begin
  //
  frmloc_cliente := tfrmloc_cliente.create(self);
  frmloc_cliente.showmodal;
  frmmodulo.qrProduto_Venda.edit;
  frmmodulo.qrProduto_Venda.fieldbyname('CODFORNECEDOR').asstring := frmmodulo.qrcliente.fieldbyname('CODIGO').asstring;
  if (frmmodulo.QrPedido_venda.State = dsinsert) then
  begin
    qrnf.close;
    qrnf.sql.clear;
    qrnf.sql.add('select * from c000141 where codfornecedor = ''' +
    ...
    begin
      ...
      begin
        bcancelarClick(frmproduto_pedido);
        frmmodulo.qrProduto_Venda.Locate('codigo', qrnf.fieldbyname('codigo')
...
procedure Tfrmproduto_Venda.efornecedorExit(Sender: TObject);
begin
  TEdit(Sender).Color := clwindow;
  if (frmmodulo.qrProduto_Venda.State = dsinsert) or
    (frmmodulo.qrProduto_Venda.State = dsedit) then
  begin
    frmmodulo.qrProduto_Venda.fieldbyname('codfornecedor').asstring :=
    ...
    begin
      ...
      begin
        if (frmmodulo.qrProduto_Venda.State = dsinsert) then
        begin
          qrnf.close;
          qrnf.sql.clear;
          qrnf.sql.add('select * from c000141 where codfornecedor = ''' +
          ...
          begin
            ...
            begin
              bcancelarClick(frmproduto_pedido);
              frmmodulo.qrProduto_Venda.Locate('codigo',
...
procedure Tfrmproduto_Venda.DBEdit1Change(Sender: TObject);
begin
  try
    qrpedido_item.close;
    qrpedido_item.sql.clear;
    qrpedido_item.sql.add('select * from c000142 where codnota = ''' +
      frmmodulo.qrProduto_Venda.fieldbyname('codigo').asstring +
      ''' order by codigo');
    qrpedido_item.open;

    if frmmodulo.qrProduto_Venda.fieldbyname('SITUACAO').asstring = '1' then
    begin
      Label7.caption := 'Aberto';
      Label7.font.Color := clTeal;
    end;
    if frmmodulo.qrProduto_Venda.fieldbyname('SITUACAO').asstring = '2' then
    begin
      Label7.caption := 'Recebido';
      Label7.font.Color := clblue;
    end;
    if frmmodulo.qrProduto_Venda.fieldbyname('SITUACAO').asstring = '3' then
    begin
...
procedure Tfrmproduto_Venda.bincluir_itemClick(Sender: TObject);
begin
  qrpedido_item.open;
  qrpedido_item.insert;
  qrpedido_item.fieldbyname('codigo').asstring := frmprincipal.codifica('000142','update');
  frmproduto_pedidoVenda_item := tfrmproduto_pedidoVenda_item.create(self);
  frmproduto_pedidoVenda_item.showmodal;

end;

procedure Tfrmproduto_Venda.balterar_itemClick(Sender: TObject);
begin
  qrpedido_item.Edit;
  frmproduto_pedidoVenda_item := tfrmproduto_pedidoVenda_item.create(self);
  frmproduto_pedidoVenda_item.showmodal;
...
procedure Tfrmproduto_Venda.DBEdit12KeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
    bgravar.setfocus;
...
procedure Tfrmproduto_Venda.rapelidoClick(Sender: TObject);
begin
  frmloc_cliente := tfrmloc_cliente.create(self);
  frmloc_cliente.showmodal;
...
procedure Tfrmproduto_Venda.Panel1Click(Sender: TObject);
begin
  // VERIFICAR
end;

procedure Tfrmproduto_Venda.DBEdit14Exit(Sender: TObject);
begin
...
procedure Tfrmproduto_Venda.BitBtn6Click(Sender: TObject);
begin
  bgravar.setfocus;
end;

procedure Tfrmproduto_Venda.FormCreate(Sender: TObject);
begin
  Label7.caption := '---';
...
procedure Tfrmproduto_Venda.qrpedido_itemCalcFields(DataSet: TDataSet);
begin
  try
...
procedure Tfrmproduto_Venda.BitBtn7Click(Sender: TObject);
begin
  if frmprincipal.autentica('Cancelar Pedido', 4) then
  begin
    if frmmodulo.QrPedido_venda.fieldbyname('situacao').asstring = '1' then
    begin
      frmmodulo.QrPedido_venda.Edit;
      frmmodulo.QrPedido_venda.fieldbyname('situacao').asstring := '3';
      frmmodulo.QrPedido_venda.post;
      frmmodulo.Conexao.commit;
    end
    else
    begin
      if frmmodulo.QrPedido_venda.fieldbyname('situacao').asstring = '3' then
      begin
        if Application.messagebox
  ...
  if frmmodulo.QrPedido_venda.fieldbyname('SITUACAO').asstring = '2' then
  begin
    Label7.caption := 'Recebido';
    Label7.font.Color := clblue;
  end;
...
procedure Tfrmproduto_Venda.BitBtn8Click(Sender: TObject);
begin
  ...
  begin
    ...
    begin
      frmmodulo.QrPedido_venda.Edit;
      frmmodulo.QrPedido_venda.fieldbyname('situacao').asstring := '2';
      frmmodulo.QrPedido_venda.post;
      frmmodulo.Conexao.commit;
    end
    else
...
procedure Tfrmproduto_Venda.BitBtn10Exit(Sender: TObject);
begin
  ptotal.visible := false;
end;

procedure Tfrmproduto_Venda.BitBtn10Click(Sender: TObject);
begin
  ptotal.visible := false;
end;

procedure Tfrmproduto_Venda.BitBtn9Click(Sender: TObject);
var
  ped, falta, rec: real;
begin
...
procedure Tfrmproduto_Venda.bitbtn5Click(Sender: TObject);
begin
  qrpedido_fornecedor.close;
  qrpedido_fornecedor.sql.clear;
  qrpedido_fornecedor.sql.add('select * from c000009 where codigo = ''' +
    frmmodulo.QrPedido_venda.fieldbyname('codfornecedor').asstring + '''');
  qrpedido_fornecedor.open;
  fxpedido.LoadFromFile('\Store_Protheus\server\rel\f000132.fr3');
  fxpedido.ShowReport;
end;

procedure Tfrmproduto_Venda.DBEdit3KeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    PERFORM(WM_NEXTDLGCTL, 0, 0);
end;

procedure Tfrmproduto_Venda.DBEdit6KeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
