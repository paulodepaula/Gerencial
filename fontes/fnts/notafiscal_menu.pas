...
uses
  ...
  ACBrNFe, ACBrNFeDANFEClass, pcnRetConsReciNFe,
  // units nfe acbr
  pcnConversaoNFe, pcnConversao, ACBrUtil, ACBrDFeUtil, SHDocVw, ComCtrls, OleCtrls, FileCtrl,
...
type
  Tfrmnotafiscal_menu = class(TForm)
    ...
    qrinventarioFARMCIA_DATA: TDateField;
    ...
    qrinventarioDATA_ABERTURA: TDateField;
    ...
    intgrfldTIPO_PAGAMENTO: TIntegerField;
...
begin
 //   := ChangeFileExt(Application.ExeName, '.ini');
   IniFile := extractfilepath(paramstr(0))+'ini\certificado.ini' ;
...
begin
  ...
  qryapoio.ParamByName('CODIGO').asstring := qrnota.fieldbyname('codigo').asstring;
  qryapoio.open;

  ACBrNFe1.NotasFiscais.clear;
      // Iniciando preenchimento da nfe
...
//      Ide.tpNF := tnSaida
    ...
    if (Qrnota.fieldbyname('CFOP').ASSTRING = '5104') or (Qrnota.fieldbyname('CFOP').ASSTRING = '5104')then
    ...
    if (query2.fieldbyname('UF').asstring = qrNFE_Cliente.fieldbyname('UF').asstring) then
    ...
    qrcompl.ParamByName('NUMNOTA').asstring := qrnota.fieldbyname('numero').asstring;
    qrcompl.open;
    ...
    if (query2.fieldbyname('IE').asstring = '') or (query2.fieldbyname('IE').asstring = 'ISENTO') then
      Emit.IE := '1234567890'
    else
    ...
    if (qrNFE_Cliente.fieldbyname('tipo').AsInteger = 2) and
       ((qrNFE_Cliente.fieldbyname('rg').asstring = 'ISENTO')  or (qrNFE_Cliente.fieldbyname('rg').asstring = ''))  then
           Begin
             Dest.IE        := 'ISENTO'; // Em BRANCO
             Dest.indIEDest :=  inIsento;
           End;

    if (qrNFE_Cliente.fieldbyname('tipo').AsInteger = 2) and (qrNFE_Cliente.fieldbyname('rg').asstring <> '')
      and (qrNFE_Cliente.fieldbyname('rg').asstring <> 'ISENTO') Then
           Begin
             Dest.IE := qrNFE_Cliente.fieldbyname('rg').asstring ;
...
//          frmmodulo.qrproduto.open;
          ...
          // NCM
         { if frmmodulo.qrproduto.Locate('codigo', qrnota_item.fieldbyname('codproduto').asstring, [loCaseInsensitive]) then
          begin
            Prod.NCM := frmPrincipal.somenteNumero(frmmodulo.qrproduto.fieldbyname('CLASSIFICACAO_FISCAL').asstring);
            Prod.CEST := frmPrincipal.somenteNumero(frmmodulo.qrproduto.fieldbyname('CEST').asstring);
          end;}
          ...
          Prod.uCom := qrnota_item.fieldbyname('UN').asstring;
          Prod.qCom := qrnota_item.fieldbyname('qtde').asfloat;
          Prod.vUnCom := qrnota_item.fieldbyname('unitario').asfloat - qrnota_item.fieldbyname('desconto').AsFloat;
          Prod.vProd := qrnota_item.fieldbyname('total').asfloat;
          Prod.qTrib := qrnota_item.fieldbyname('qtde').asfloat;
          Prod.vUnTrib := qrnota_item.fieldbyname('unitario').asfloat - qrnota_item.fieldbyname('desconto').AsFloat;
          Prod.uTrib := qrnota_item.fieldbyname('UN').asstring;
          Prod.vDesc := 0.00;// qrnota_item.fieldbyname('desconto').AsCurrency;
          Prod.vFrete := qrnota.fieldbyname('frete').AsCurrency;
          Prod.vSeg := qrnota.fieldbyname('seguro').AsCurrency;
          Prod.vOutro := qrnota.fieldbyname('outras_despesas').AsCurrency;
      ...
      // valor dos produtos
      Total.ICMSTot.vProd := qrnota.fieldbyname('VALOR_PRODUTOS').asfloat - qrnota.fieldbyname('DESCONTO').asfloat;
      // desconto
      Total.ICMSTot.vDesc := 0 ;// qrnota.fieldbyname('DESCONTO').asfloat;
     ...
     // contas a pagar
      ...
      cobr.Fat.vOrig := RoundTo(qrnota.fieldbyname('VALOR_PRODUTOS').asfloat, -2);
      cobr.Fat.vDesc := RoundTo(qrnota.fieldbyname('DESCONTO').asfloat, -2); //produção
...
//    end;
    ...
    InfAdic.infCpl := InfAdic.infCpl + #13 + 'Total Aprox Tributos R$' +
      FormatFloat('##,###,##0.00', TotalIBPT) + ' (' +
      FormatFloat('0.00', PorcIBPT) + '%) Fonte: IBPT';

     with pag.add do // PAGAMENTOS apenas para NFC-e
        begin
          case qrnota.fieldbyname('tipo_pagamento').asinteger of
            0:
              tPag  := fpDinheiro;
            1:
              tPag  := fpCheque;
          ...
          vPag := qrnota.fieldbyname('total_nota').AsCurrency; //total_produtos + rTotal_Acrescimo - rTotal_Desconto;
   ...
   // ACBrNFe1.NotasFiscais.GerarNFe;
    ...
    {  try
     ...
     showmessage(ACBrNFe1.NotasFiscais.Items[0].NomeArq); }
     ACBrNFe1.NotasFiscais.GerarNFe;

 // except }
      //  result := ACBrNFe1.NotasFiscais.Items[0].NomeArq; // extractfilepath(paramstr(0)) + 'nfe\Geradas\'+ ACBrNFe1.NotasFiscais.Items[0].NomeArq;
     //   showmessage(ACBrNFe1.NotasFiscais.Items[0].NomeArq);
...
begin
  ...
  qrnota.Insert;
  qrnota.fieldbyname('codigo').asstring := FrmPrincipal.codifica('000061','ler');//frmModulo.codificanfe(SERIE,'55','LER');
  ...
  // Verifica configuraoes - Gerar contas a receber ou não gerar
  frmmodulo.qrconfig.open;
             ...
             begin
                   if FileExists(sXML) then
                     begin
                       ...
                       begin
                            Application.messagebox('Arquivo XML nao encontrado!', 'Erro', mb_ok + mb_iconerror);
                       end;
             end;
       except
           on E: exception do
  ...
  try
    ...
    if FileExists(sXML) then
       begin
        ...
        qrnota.fieldbyname('chave').AsString := Copy(ACBrNFe1.NotasFiscais.Items[0].NFe.infNFe.ID,
...
begin
  ...
  if sXML = '' then
    ...
    try
      CC.Add('storeprotheus@gmail.com'); //especifique um email vÃ¡lido
      CC.Add('storeprotheus@gmail.com');    //especifique um email vÃ¡lido
 ...
 with ACBrNFe1.EventoNFe.Evento.Add do
   begin
     ...
     if  nSeqEvento = 21 then
       begin
        ShowMessage('Numero maxímo de correções por nota são 20, '+#13+' não são possiveis novas correções nesta nota '+#13+ inttostr(qrnota.fieldbyname('seqevento').AsInteger)+ ' correções ja efetuadas');
        exit;
       end
       else
       begin
