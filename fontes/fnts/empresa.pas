...
begin
  if frmprincipal.acesso(codigo_usuario, '01.01') = 'LIMITADO' then
   begin
    ...
    frmmodulo.QrM55.close;
    frmmodulo.QrM55.sql.clear;
    frmmodulo.QrM55.sql.add('Select '''+ Serie+ ''', numero from C000139 where modelo = ''55''');
...
begin
  ...
  frmmodulo.QRFILIAL.fieldbyname('codigo').asstring := frmprincipal.codifica('000004','update');
...
procedure Tfrmempresa.EdtSerieEnter(Sender: TObject);
begin
  tedit(sender).Color := $00A0FAF8;
end;

procedure Tfrmempresa.EdtSerieExit(Sender: TObject);
begin
  tedit(sender).Color := clwindow;
end;

procedure Tfrmempresa.EdtSerieKeyPress(Sender: TObject; var Key: Char);
begin
  if key = #13 then perform(wm_nextdlgctl, 0, 0);
end;
...
begin
  ...
  begin
    ...
    FRMMODULO.QRemitente.FIELDBYNAME('logo').Value :=  OpenPictureDialog1.Filename;
  end;
