...
uses
  ...
  ACBrBoletoFCFR, ACBrBase, Uni, Data.DBXSybaseASA,interbaseuniprovider; // NeoKeyUnit,frxExportImage,frxExportRTF,
...
type
  Tfrmmodulo = class(TDataModule)
    ...
    qr_cstNacional: TZQuery;
    qr_cstNacionalCODIGO: TWideStringField;
    qr_cstNacionalSITUACAO_TRIBUTARIA: TWideStringField;
    qr_cstNacionalENTRADA_SAIDA: TWideStringField;
    qrprodutoATIVA: TWideStringField;
    qrprodutoPIS: TFloatField;
    qrprodutoCOFINS: TFloatField;
    qrprodutoCST_NACIONAL: TWideStringField;
    qrprodutoDATA_VENCIMENTO: TDateField;
    qrSangrias: TZQuery;
    CDSoscliente_celular: TStringField;
    wdstrngfldCOD_OS_ORIGINAL: TWideStringField;
    query_servidor: TZQuery;
    con2: TUniConnection;
    intgrfldVENDA_FECHAMENTO_AVULSO: TIntegerField;
    QrPedido_venda: TZQuery;
    ...
    qrProduto_Venda: TZQuery;
    wdstrngfldProduto_VendaCODIGO: TWideStringField;
    wdstrngfldProduto_VendaNUMERO: TWideStringField;
    wdstrngfldProduto_VendaCFOP: TWideStringField;
    dtfldProduto_VendaDATA: TDateField;
    wdstrngfldProduto_VendaCODFORNECEDOR: TWideStringField;
    fltfldProduto_VendaTOTAL_NOTA: TFloatField;
    intgrfldProduto_VendaITENS: TIntegerField;
    intgrfldProduto_VendaSITUACAO: TIntegerField;
    dtfldProduto_VendaPREVISAO: TDateField;
    wdstrngfldProduto_VendaCONDPGTO: TWideStringField;
    wdstrngfldProduto_VendaOBS1: TWideStringField;
    wdstrngfldProduto_VendaOBS2: TWideStringField;
    wdstrngfldProduto_VendaOBS3: TWideStringField;
    wdstrngfldProduto_VendaOBS4: TWideStringField;
    QrM55: TZQuery;
    wdstrngfldSOLICITANTE: TWideStringField;
...
//    procedure Busca_Integridade_Base_Dados;
  ...
  public
    ...
    function codificanfe(SERIE, MODELO, ACTION :string): string;
...
function Tfrmmodulo.Zerar(texto: string; qtde: Integer): string;
begin
...
function Tfrmmodulo.codificanfe(SERIE, MODELO, ACTION : string): string;
var
icod : Integer;
scod : string;
begin
  qrm55.close;
  qrm55.SQL.clear;
  qrm55.SQL.add('select * from c000139 where serie = ''' + SERIE +''' and modelo = ' + modelo + ' WITH LOCK');
  qrm55.open;
  if qrm55.recordcount > 0 then
    begin
      scod := qrm55.FIELDBYNAME('numero').AsString;
      if StrToInt(scod) < 1 then
        scod := '1';
      if action = 'update' then
        begin
          ...
          qrm55.close;
          qrm55.SQL.clear;
          qrm55.SQL.add('update c000139 set numero = '+ scod+ ' where serie = ''' + SERIE +''' and modelo = ' + modelo);
          qrm55.execsql;
          CONEXAO.COMMIT;
          result := frmPrincipal.zerarcodigo(scod, 6);
...
var
  ...
  porta, i: Integer;
begin
    // FUNCAO PARA PROTECAO NEOKEY na versao 2.0 do Store_Protheus
  begin
    // LOGIN
  ...
  //caminho := '\Store_Protheus\server\ini\com.ini'; // Abre o Sistema Atual
  caminho := sPath_ini + 'com.ini';
  ...
  begin
    ...
    begin
      if trim(copy(entrada, 9, 50)) = 'LOCALHOST' then
        frmprincipal.caption := 'RDT Gerencial 10.0'
      else
        frmprincipal.caption := 'RDT Gerencial 10.0                 [ Servidor: ' +
...
begin
  qrLog.FieldValues['codigo'] := frmprincipal.codifica('000002','update');
end;
...
begin
  ...
  begin
    frmmodulo.qrsubgrupo.fieldbyname('codigo').asstring :=
      frmprincipal.codifica('000018','update');
...
begin
  ...
  begin
    frmmodulo.qrgrade_subgrupo.fieldbyname('codigo').asstring :=
      frmprincipal.codifica('000020','update');
...
begin
  ...
  frmmodulo.qrNF_CTE.fieldbyname('codigo').asstring :=
    frmprincipal.codifica('NF_CTE','update');
end;
...
begin
  qragenda.FieldValues['codigo'] := frmprincipal.codifica('000034','update');
end;
