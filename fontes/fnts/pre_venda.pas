...
uses
  ...
  RzLabel, jpeg, ExtCtrls, Menus, AdvMenus, DB, DBAccess, pngimage, dxGDIPlusClasses, Uni,
  MemDS;
...
type
  TfrmPre_Venda = class(TForm)
   ...
   // qrPre_Venda: TIBCQuery;
    ...
    function zerar(codigo: string; Qtde: Integer): string;
  private
...
uses
principal, msg_Operador, prevenda_conciliar, venda,modulo,
senha_supervisor, menu_fiscal ; // menu_fiscal,
...
function TfrmPre_Venda.zerar(codigo: string; Qtde: Integer): string;
begin
...
begin
  ...
  frmmodulo.query_servidor.SQL.Add('select orc.*, prod.produto, prod.unidade, prod.codbarra, prod.cst, prod.comissao, prod.aliquota');
  ...
  sCodNota := grid_venda.cell[0,i].asstring;
  ...
  begin
    ...
    while not frmmodulo.query_servidor.eof do
     begin
      ...
      FRMVENDA.qrvenda_produto.FieldByName('codproduto').AsString :=  frmmodulo.query_servidor.fieldbyname('codproduto').AsString;  // qrproduto.FieldByName('codigo').AsString;
      FRMVENDA.qrvenda_produto.FieldByName('codbarras').AsString :=  frmmodulo.query_servidor.fieldbyname('codbarra').AsString ; // qrproduto.FieldByName('codbarra').AsString;
      FRMVENDA.qrvenda_produto.FieldByName('produto').AsString :=     frmmodulo.query_servidor.fieldbyname('produto').AsString;
      FRMVENDA.qrvenda_produto.FieldByName('NUMERACAO').AsString :=   frmmodulo.query_servidor.fieldbyname('unidade').AsString;
      FRMVENDA.qrvenda_produto.FieldByName('ALIQUOTA').asfloat :=     frmmodulo.query_servidor.fieldbyname('ALIQUOTA').asfloat;
      FRMVENDA.qrvenda_produto.FieldByName('CST').AsString :=     frmmodulo.query_servidor.fieldbyname('CST').AsString;
      FRMVENDA.qrvenda_produto.FieldByName('terminal').AsString :=       numero_terminal;
      FRMVENDA.qrvenda_produto.FieldByName('qtde').asfloat := frmmodulo.query_servidor.fieldbyname('qtde').asfloat;
      FRMVENDA.qrvenda_produto.FieldByName('unitario').asfloat := frmmodulo.query_servidor.fieldbyname('unitario').asfloat;
      FRMVENDA.qrvenda_produto.FieldByName('total').asfloat := frmmodulo.query_servidor.fieldbyname('TOTAL').asfloat;
      FRMVENDA.qrvenda_produto.FieldByName('desconto').asfloat :=   frmmodulo.query_servidor.fieldbyname('DESCONTO').asfloat;
     ...
     frmvenda.lvenda_codigo.caption :=  sCodNota;
    close;
  end;
end;
...
procedure TfrmPre_Venda.CentralizarPanel(p:TPanel);
begin
...
procedure TfrmPre_Venda.FormResize(Sender: TObject);
begin
