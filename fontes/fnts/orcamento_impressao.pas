...
type
  Tfrmorcamento_impressao = class(TForm)
    ...
    procedure fxorcamentoGetValue(const VarName: string; var Value: Variant);
  private
...
procedure Tfrmorcamento_impressao.fxorcamentoGetValue(const VarName: string;
  var Value: Variant);
begin
if VarName = 'Fantasia' then
   if FrmOrcamento.chk1.checked then
    value :=  FrmOrcamento.edtSolicitante.text
    else
    value :=  FrmOrcamento.enomecliente.text;
end;
...
begin
  ...
  // gera NF
    ...
    qrnotafiscal.FieldByName('codigo').asstring := frmprincipal.codifica('000061','update');
    ...
    if frmmodulo.qrorcamento.FieldByName('CODVENDA').AsString = '' then xproxvenda := frmprincipal.codifica('000048','update');
...
begin
  ...
  // gera NF
    ...
    frmmodulo.qrorcamento.edit;
    frmmodulo.qrorcamento.FieldByName('CODVENDA').AsString := frmprincipal.codifica('000048','update');
    frmmodulo.qrorcamento.post;
...
begin
 ...
 //  fxorcamento.Variables.Clear;
   fxorcamento.Variables['Fantasia'] := 'Show room';
 // if chk1.checked then

  frmmodulo.qrrelatorio.post;
  fxorcamento.LoadFromFile('\Store_Protheus\server\rel\f000069.fr3');
