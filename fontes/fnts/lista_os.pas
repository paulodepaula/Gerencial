...
begin
  ...
  if combo_veiculo.Text = 'TODOS' then
   ...
   begin
    veiculo := ' and codveiculo = ''' + copy(combo_veiculo.text, 1, 6) + '''';
  ...
  if combo_tipo.Text = 'TODAS' then
   ...
   begin
    TIPO := ' and tipo like ''%' + combo_tipo.text + '%''';
  ...
  if combo_setor.Text = 'TODOS' then
    ...
    begin setor := ' and codsetor = ''' + copy(combo_setor.text, 1, 6) + '''';
  ...
  begin
    ...
    fXOS.ShowReport;
                 ...
                 //******************** RELAÇÃO DE O.S. AGRUPADA POR TÉCNICO *************************
  ...
  begin
    if combo_tecnico.Text = 'TODOS' then
     tecnico := ''
      ...
      begin tecnico := ' and CODTECNICO = ''' + copy(combo_tecnico.text, 1, 6) + '''';
       frmmodulo.qrrelatorio.fieldbyname('LINHA4').asstring := 'TÉCNICO: ' + COMBO_TECNICO.TEXT;
      end;
    ...
    frmmodulo.qrservicos_periodicos.ParamByName('datai').asdatetime := dateedit1.date;
    frmmodulo.qrservicos_periodicos.ParamByName('dataf').asdatetime := dateedit2.date;
  ...
  //*******************RELAÇÃO DE SERVIÇOS AGRUPADOS POR TÉCNICO ****************
  ...
  begin
    ...
    if combo_tecnico.Text = 'TODOS' then
     ...
     begin tecnico := ' and tec.codigo = ''' + copy(combo_tecnico.text, 1, 6) + '''';
      frmmodulo.qrrelatorio.fieldbyname('LINHA4').asstring := 'TÉCNICO: ' + COMBO_TECNICO.TEXT;
     end;
    ...
    if combo_veiculo.Text = 'TODOS' then
     ...
     begin
      veiculo := ' and os.codveiculo = ''' + copy(combo_veiculo.text, 1, 6) + ''''; frmmodulo.qrrelatorio.fieldbyname('LINHA5').asstring := 'VEÍCULO ' + COMBO_VEICULO.TEXT + '   ';
     end;

    if combo_tipo.Text = 'TODAS' then
     TIPO := ''
    else
     begin TIPO := ' and os.tipo like ''%' + combo_tipo.text + '%''';
      frmmodulo.qrrelatorio.fieldbyname('LINHA5').asstring := frmmodulo.qrrelatorio.fieldbyname('LINHA5').asstring + 'TIPO: ' + COMBO_TIPO.TEXT;
     end;
     ...
     if combo_atendimento.Text = 'TODOS' then
      ...
      begin atendimento := ' and os.atendimento like ''%' + combo_atendimento.text + '%''';
       frmmodulo.qrrelatorio.fieldbyname('LINHA5').asstring := frmmodulo.qrrelatorio.fieldbyname('LINHA5').asstring + 'Atendimento: ' + COMBO_atendimento.TEXT;
      end;
    ...
    if combo_SITUACAO.Text = 'TODAS' then
     ...
     if combo_periodo.text = 'DATA DA EMISSÃO' then
      begin
          periodo := ' os.conclusao_data >= :datai and os.conclusao_data <= :dataf ';
          ordem := 'conclusao_data,codigo';
          frmmodulo.qrrelatorio.fieldbyname('LINHA2').asstring := 'PERÍODO DE FECHAMENTO: ' + DATEEDIT1.TEXT + ' A ' + DATEEDIT2.TEXT;
      end
       else
       begin
          periodo := ' serv.data >= :datai and serv.data <= :dataf ';
          ordem := 'data, codigo';
          frmmodulo.qrrelatorio.fieldbyname('LINHA2').asstring := 'PERÍODO DE ABERTURA: ' + DATEEDIT1.TEXT + ' A ' + DATEEDIT2.TEXT;
       end;
    ...
    qros_servico.sql.add('select serv.*,(serv.valor - serv.desc_parcial) as Valor_Final,'+
                         ' os.situacao, nserv.*, cli.nome,Tec.nome tecnico, tec.comissao');
    ...
    qros_servico.sql.add('and '+ periodo);
    qros_servico.sql.add(cliente + tecnico + veiculo + tipo + situacao + setor + atendimento);
    qros_servico.SQL.add('order by codtecnico,data,os.codigo');
   ...
   // showmessage(qros_servico.SQL.text);
    fXOS.ShowReport;
