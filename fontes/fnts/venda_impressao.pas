...
uses
  ...
  frxDesgn, frxClass, frxDBSet, AdvGlowButton, AdvShapeButton, DBAccess, Mask, RzEdit, RzSpnEdt,
...
uses modulo, principal, venda, venda_fechamento2, venda_fechamento, ecf, venda_impressaocp,//venda_cupom,
...
var
  f, arquivo: textfile;
  codi, TEXTO8, texto, texto1, texto2, texto3, texto4, texto5, texto6, texto7: string;
  i, a: integer;
  ...
  xtotserv, desconto_nota, soma_desconto,rSubtotal : real;
begin
...
// if (FRMMODULO.QRCONFIG.FieldByName('CADASTRO_PRODUTO').asstring = 'NORMAL') or (FRMMODULO.QRCONFIG.FieldByName('CADASTRO_PRODUTO').asstring = 'LIGHT') or (FRMMODULO.QRCONFIG.FieldByName('CADASTRO_PRODUTO').asstring = 'FARMA') then
     ...
     try
       if VendaFechamento then  // traz desconto do frmvenda_fechamento
         begin
          desconto_nota := frmvenda_fechamento.rdesconto2.value ;
          rSubtotal := frmvenda_fechamento.rSubtotal.value ;
         end
       else
         begin
          desconto_nota := frmvenda_fechamento2.rdesconto2.value ;
          rSubtotal := frmvenda_fechamento2.rSubtotal.value ;
         end;
        qrcliente.close;
        qrcliente.sql.clear;
        ...
        qrnotafiscal.open;
      //  showmessage(frmmodulo.codificanfe(SERIE,'55','update'));
        qrnotafiscal.insert;
        qrnotafiscal.FieldByName('codfilial').asstring := '000001';
        qrnotafiscal.FieldByName('natureza').asstring := frmmodulo.natoperacao;
        codi := frmPrincipal.codifica('000061','update');
        qrnotafiscal.FieldByName('codigo').asstring := codi;
        qrnotafiscal.FieldByName('Numero').asstring := frmmodulo.codificanfe(SERIE,'55','update');
        ...
        numero_notaz := frmprincipal.zerarcodigo(qrnotafiscal.FieldByName('Numero').asstring, 6);
        // correcoes
        ...
        frmmodulo.qrfilial.open;
        qrnotafiscal.FieldByName('SERIE_NF').asstring := SERIE;
        ...
        qrnotafiscal.FieldByName('TIPO_PAGAMENTO').asInteger := iTipoPgto; // AQUI
        ...
        tn := 0.00;
          ...
          while not frmvenda.qrvenda_produto.Eof do
            begin
              ...
              begin
                ...
                if xitemserv = 1 then
                begin
                  qrnotafiscal.FieldByName('OS_COD1').asstring := frmvenda.qrvenda_produto.FieldByName('CODPRODUTO').asstring;
                  qrnotafiscal.FieldByName('OS_SERV1').asstring := frmvenda.qrvenda_produto.FieldByName('PRODUTO').asstring;
                  qrnotafiscal.FieldByName('OS_QTDE1').asfloat := frmvenda.qrvenda_produto.FieldByName('QTDE').asfloat;
                  qrnotafiscal.FieldByName('OS_TOTAL1').asfloat := frmvenda.qrvenda_produto.FieldByName('TOTAL').asfloat;
                  qrnotafiscal.FieldByName('OS_TOTAL_GERAL').asfloat := frmvenda.qrvenda_produto.FieldByName('TOTAL').asfloat;
                  qrnotafiscal.FieldByName('OS_UNIT1').asfloat := frmvenda.qrvenda_produto.FieldByName('unitario').asfloat;
          ...
          begin
            ...
            qrnotafiscal_item.FieldByName('UNITARIO').asfloat := frmvenda.qrvenda_produto.FieldByName('TOTAL').asfloat /
              frmvenda.qrvenda_produto.FieldByName('QTDE').asfloat;
            ...
            begin
              ...
              begin
                ...
                qrapoio.FieldByName('unitario').asstring := formatfloat('###,###,##0.0000',
          ...
          begin
           // Inclusão de produto na emisão de NFE
            ...
            begin
              ...
              begin
                ...
                qrnotafiscal_item.FieldByName('Item').asInteger := frmvenda.qrvenda_produto.FieldByName('ITEM').asInteger;
                qrnotafiscal_item.FieldByName('CODNOTA').asstring := qrnotafiscal.FieldByName('CODIGO').asstring;
                qrnotafiscal_item.FieldByName('CODPRODUTO').asstring := frmvenda.qrvenda_produto.FieldByName('CODPRODUTO').asstring;
                qrnotafiscal_item.FieldByName('QTDE').asfloat := frmvenda.qrvenda_produto.FieldByName('QTDE').asfloat;
                {
                if qrvenda.FieldByName('desconto').asfloat > 0 then
                 qrnotafiscal_item.FieldByName('desconto').asfloat := frmvenda.qrvenda_produto.FieldByName('desconto').asfloat +
                  (qrvenda.FieldByName('desconto').asfloat / frmvenda.qrvenda_produto.recordcount)
                 else
                   qrnotafiscal_item.FieldByName('desconto').asfloat := frmvenda.qrvenda_produto.FieldByName('desconto').asfloat;
                }
                qrnotafiscal_item.FieldByName('subtotal').asfloat := frmvenda.qrvenda_produto.FieldByName('Unitario').asfloat - frmvenda.qrvenda_produto.FieldByName('desconto').asfloat;
                qrnotafiscal_item.FieldByName('UNITARIO').asfloat := frmvenda.qrvenda_produto.FieldByName('unitario').asfloat;
                qrnotafiscal_item.FieldByName('TOTAL').asfloat := frmvenda.qrvenda_produto.FieldByName('TOTAL').asfloat;

                if frmmodulo.qrproduto.Locate('CODIGO',frmvenda.qrvenda_produto.FieldByName('CODPRODUTO').asstring,[loCaseInsensitive]) then
                 begin
                ...
                soma_desconto := soma_desconto + qrnotafiscal_item.FieldByName('desconto').asfloat;

                if desconto_nota > 0 then
                 begin
                  qrnotafiscal_item.FieldByName('desconto').asfloat :=  qrnotafiscal_item.FieldByName('desconto').asfloat +
                    (desconto_nota * (((qrnotafiscal_item.FieldByName('TOTAL').asfloat * 100) / rSubtotal) / 100));
                  qrnotafiscal_item.FieldByName('TOTAL').asfloat := qrnotafiscal_item.FieldByName('UNITARIO').asfloat *
                    qrnotafiscal_item.FieldByName('QTDE').asfloat - qrnotafiscal_item.FieldByName('desconto').asfloat;
                 end;
                  ...
                  begin
                    if qrnotafiscal_item.FieldByName('total').asfloat > 0 then
                      begin
                       ...
                       qrapoio.FieldByName('unitario').asstring := formatfloat('###,###,##0.00',
                        qrnotafiscal_item.FieldByName('subtotal').asfloat);
                      end
                    else
                     if qrnotafiscal_item.FieldByName('total').asfloat > 0 then
          ...
          qrnotafiscal.close;
          qrnotafiscal.sql.clear;
          qrnotafiscal.sql.add('select * from  c000061  where codigo = '+quotedstr(codi) );
          qrnotafiscal.open;

          if soma_desconto > 0 then
            begin
              qrnotafiscal.edit;
              qrnotafiscal.fieldbyname('desconto').asFloat := soma_desconto ;
              qrnotafiscal.post;
            end;
...
begin
  frmmodulo.gera_nf := 0;
  frmmodulo.Conexao.Commit;
 // frmvenda_cupom := tfrmvenda_cupom.Create(self);
 // frmvenda_cupom.showmodal;
  hide;
...
begin
  ...
  frmmodulo.qrnotafiscal.FieldByName('codigo').asstring :=
    frmprincipal.codifica('000061','update');
  frmmodulo.qrfilial.open;
  frmmodulo.qrfilial.Locate('codigo', '000001', [loCaseInsensitive]);
  ...
  begin
    ...
    frmmodulo.qrecf_comando.FieldByName('sequencia').asstring :=
      frmprincipal.codifica('000072','update');
