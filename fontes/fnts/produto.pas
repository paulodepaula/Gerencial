...
uses
  ...
  JvBaseEdits, AdvSmoothButton, JvMaskEdit, JvCheckedMaskEdit, JvDatePickerEdit,
  JvDBDatePickerEdit;
...
type
  Tfrmproduto = class(TForm)
    ...
    Label101: TLabel;
    Label102: TLabel;
    ECFOPDENTRO: TRzDBButtonEdit;
    DBEdit73: TDBEdit;
    ECFOPFORA: TRzDBButtonEdit;
    DBEdit74: TDBEdit;
    ...
    lbl3: TLabel;
    lbl6: TLabel;
    DBe_CST_NACIONAL: TRzDBButtonEdit;
    lbl2: TLabel;
    dbdateVALIDADE: TJvDBDatePickerEdit;
    Label89: TLabel;
    ...
    procedure DBe_CST_NACIONALButtonClick(Sender: TObject);
    procedure DBe_CST_NACIONALKeyPress(Sender: TObject; var Key: Char);
...
uses modulo, principal, loc_grupo,
  ...
  xloc_produto, produto_atualizapreco, xloc_cst,xloc_cst_PC, inventario, baixa_estoque, servico, loc_aliquota, Produto_consultaserial,
...
//  Dbedit1.text := FrmPrincipal.codifica('000025','ler');
  ...
  qrproduto.FieldByName('codigo').AsString := frmprincipal.codifica('000025','ler');

  edata_cadastro.date := date;

  if (qrproduto.State = dsedit) or (qrproduto.State = dsinsert) then
     PageView2.enabled := true;
...
begin
  ...
  if (eNCM.text = '') or (length(eNCM.text) < 8) or (length(eNCM.text) > 8) then
   begin
    PageView1.ActivePageIndex := 0;
    PageView2.ActivePageIndex := 0;
    eNCM.setfocus;
    showmessage('Verificar o NCM se atende as expecificações de 8 Digitos!');

   end;

  if (ECFOPDENTRO.TEXT = '') OR (ECFOPFORA.TEXT = '') then
   begin
    PageView1.ActivePageIndex := 0;
    PageView2.ActivePageIndex := 3;
    ecfopdentro.setfocus;
    showmessage('Verifcar o CFOP adequado ao produto ! ');
   end;
...
//    FRMMODULO.qrinfnutricional_produto.Post;
  ...
  if (qrproduto.State = dsinsert) then
    qrproduto.FieldByName('codigo').AsString :=  frmprincipal.codifica('000025','update');

  qrproduto.Post;
  with FRMMODULO.qrinfnutricional_produto do
    begin
      if (state = dsinsert) then
        begin
         frmprincipal.codifica('000028','update');
         FRMMODULO.qrinfnutricional_produto.post;
        end;
...
begin
  ...
  qrpreco.insert;
  qrpreco.FieldByName('codigo').AsString := frmprincipal.codifica('000026','update');
    ...
    begin
      ...
      begin
        ...
        FRMMODULO.qrgrade_produto.FieldByName('codigo').AsString :=
          frmprincipal.codifica('000021','update');
        ...
        begin
          ...
          begin
            ...
            qrproduto_mov.FieldByName('codigo').AsString :=
              frmprincipal.codifica('000032','update');
...
begin
  qrcomposicao_produto.FieldByName('codigo').AsString :=
    frmprincipal.codifica('000027','update');
...
begin
  ...
  FRMMODULO.qrinfnutricional_produto.FieldByName('codigo').AsString := frmprincipal.codifica('000028','ler');
...
begin
  if (ealiq.Value <> 0.00) and (ealiq.Value <> 7.00) and (ealiq.Value <> 12.00)
    and (ealiq.Value <> 17.00) and (ealiq.Value <> 18.00) and (ealiq.Value <> 17.50) and
...
begin
  ...
  begin
    ...
    qrcodbarra.FieldByName('CODIGO').AsString :=
      frmprincipal.codifica('000055','update');
...
procedure Tfrmproduto.btn1Click(Sender: TObject);
begin
 FrmLote := TFrmLote.Create(self);
 //query Para Mostrar Ultima entrada do produto
FrmLote.QrEstoque.Close;
FrmLote.QrEstoque.SQL.Clear;
FrmLote.QrEstoque.SQL.Add('select maxvalue(c000100.ultima_entrada) as Ultima_entrada, c000100.* from c000100 '  +
                          'where c000100.codproduto = '+ DBEdit1.Text );
QrEstoque.Open;
//Fim query mostrar ulta entrada
 FrmLote.DbdataEntrada.Date := QrEstoque.FieldByName('Ultima_entrada').Value;
 FrmLote.LbCod.Caption:= DBEdit1.Text;
 FrmLote.LbProd.Caption := DBEdit2.Text;
 FrmLote.dbDataVenci.date := QrEstoque.FieldByName('Ultima_entrada').Value + 10;
  ...
  begin
    ...
    begin
      ...
      QRRENTABILIDADE.FieldByName('codigo').AsString :=
        frmprincipal.codifica('000078','update');
...
begin
  ...
  begin
    ...
    FRMMODULO.qradic.sql.add('values');
    FRMMODULO.qradic.sql.add('(''' + frmprincipal.codifica('000032','update') +
  ...
  begin
    ...
    begin
      ...
      if eCEST.Text <> '000000' then
        ...
        begin
          application.messagebox('CEST não encontrado!', 'Atenção',
            mb_ok + mb_iconerror);
          eCEST.SetFocus;
          exit;
        end
...
procedure Tfrmproduto.DBe_CST_NACIONALButtonClick(Sender: TObject);
begin
if qrproduto.State <> dsinsert then
    if qrproduto.State <> dsinsert then
      qrproduto.Edit;

  FRMXLOC_CST_PC := TFRMXLOC_CST_PC.create(self);
  FRMXLOC_CST_PC.showmodal;
  qrproduto.FieldByName('CST_NACIONAL').AsString := resultado_pesquisa1;
  DBEDTPIS.SetFocus;
end;

procedure Tfrmproduto.DBe_CST_NACIONALKeyPress(Sender: TObject; var Key: Char);
begin
if Key = #13 then
  begin
    if (qrproduto.State = dsinsert) or (qrproduto.State = dsedit) then
    begin
      if dbedtPIS.Text <> '' then
      begin
        FRMMODULO.qr_cstnacional.close;
        FRMMODULO.qr_cstnacional.sql.clear;
        FRMMODULO.qr_cstnacional.sql.add('SELECT * FROM C000133 WHERE CODIGO = '''
          + dbedtPIS.Text + '''');
        FRMMODULO.qr_cstnacional.OPEN;
        if FRMMODULO.qr_cstnacional.RECORDCOUNT > 0 then
        begin
          qrproduto.FieldByName('CST_NACIONAL').AsString := FRMMODULO.qr_cstnacional.FieldByName('codigo').AsString;
          Dbedtpis.SetFocus;
        end
        else
        begin
        ...
        DBe_Cst_nacionalButtonClick(frmproduto);
    end;
      ...
      begin
        ...
        begin
          ...
          QUERY1.FieldByName('codigo').AsString :=
            frmprincipal.codifica('000026','update');
          QUERY1.FieldByName('codproduto').AsString :=
