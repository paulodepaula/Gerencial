...
type
  Tfrmnotafiscal = class(TForm)
    ...
    procedure qrnotafiscal_item3NewRecord(DataSet: TDataSet);
...
uses modulo, principal, loc_cfop, Math, loc_cliente, notafiscal_item,
  ...
  notafiscal_importacupom, sintegra_gerar, OpNatureza, URemessa;
...
begin
  ...
  if (edit5.text = '') or (edit5.text = '000000') then
  begin
 ...
 if (edata_saida.text = '  /  /    ')  then
  ...
  if (frmnotafiscal_menu.qrnota.State = dsinsert) then
    begin
     frmnotafiscal_menu.qrnota.fieldbyname('codigo').asstring := FrmPrincipal.codifica('000061','update');
    end
  else
  begin
    // frmnotafiscal_menu.qrnota.fieldbyname('codigo').asstring := FrmPrincipal.codifica('000061','ler');
  end;
 ...
 try
  ...
  vdesconto_atual := 0;
  vtotal_desconto := frmnotafiscal_menu.qrnota.fieldbyname('DESCONTO').AsFloat ;
  ...
  begin
    ...
    begin
      ...
      if frmnotafiscal_menu.qrnota.State = dsinsert then  //grava somente se for novo registro "MODO INSERT"
        begin
          ...
          frmmodulo.qrfilial.fieldbyname('notafiscal').asinteger := StrToInt(FrmModulo.codificanfe(SERIE,'55','update'));
          ...
          while not qrnotafiscal_item.eof do
            ...
            // begin
              ...
              frmnotafiscal_menu.qrnota.fieldbyname('valor_isento_icms').AsFloat + qrnotafiscal_item.fieldbyname('ISENTO').AsFloat;
      ...
      else // GRAVA SOMENTE EM MODO EDIT NO BANCO
      begin (* *********  editar a tabela de movimento  *********** *)
        ...
        qrnotafiscal_item.first;
        while not qrnotafiscal_item.eof do   // ATUALIZA ITEM DA NOTA
          begin
            ...
            if qrnotafiscal_item.fieldbyname('VALOR_ICMS').AsFloat > 0 then
              frmnotafiscal_menu.qrnota.fieldbyname('valor_ICMS').AsFloat :=
                ...
                (frmnotafiscal_menu.qrnota.fieldbyname('seguro').AsFloat / vitem)
                + (frmnotafiscal_menu.qrnota.fieldbyname('outras_despesas')
                .AsFloat / vitem))) * qrnotafiscal_item.fieldbyname('ICMS').AsFloat / 100;
            ...
            begin
              ...
              if (frmnotafiscal_menu.qrnota.fieldbyname('desconto').AsFloat > 0)
                and (check_desconto.Checked = false) and (frmnotafiscal_menu.qrnota.State = dsinsert) then
               begin
                query.close;
                query.sql.clear;
    ...
    frmnotafiscal_menu.qrnota.fieldbyname('valor_produtos').AsFloat :=  frmprincipal.Arredondar(frmnotafiscal_menu.qrnota.fieldbyname
  ...
  //  frmnotafiscal_menu.qrnota.post;
    ...
    qrapoio.parambyname('numero').asstring := Edit5.text;
  ...
  begin
    ...
    begin
      ...
      begin
        frmmodulo.qrfilial.edit;
        frmmodulo.qrfilial.fieldbyname('notafiscal').asinteger := StrToInt(edit5.text) + 1;
        // frmmodulo.qrfilial.fieldbyname('notafiscal').asinteger + 1;
        frmmodulo.qrfilial.post;
      end;
  ...
  begin
    ...
    begin
      ...
      begin
        if frmnotafiscal_menu.qrnota.State = dsinsert then
        begin
          frmnotafiscal_menu.qrnota.fieldbyname('numero').asstring := frmmodulo.codificanfe(SERIE,'55','ler');
 ...
 //   frmnotafiscal_menu.qrnota.fieldbyname('numero').asstring :=  frmprincipal.zerarcodigo(ENUMERO.text, 6);
  ...
  begin
    ...
    begin
      if (frmnotafiscal_menu.qrnota.State = dsinsert) then
      begin
        if application.messagebox('Existe uma nota fiscal já emitida com este número! Deseja prosseguir?',
          'Atenção', mb_yesno + mb_iconwarning + MB_DEFBUTTON2) = idno
        then
...
begin
  ...
  if (Edit3.text = Edit4.text) and (copy(ecfop.text, 1, 1) = '6') then
   ...
   if (Edit3.text <> Edit4.text) and (copy(ecfop.text, 1, 1) = '5') then
   begin
    showmessage
       ...
       // ecliente.text := FrmPrincipal.zerarcodigo(ecliente.text,6) ;
        ...
        if frmmodulo.qrcliente.RecordCount = 0 then
          ...
          begin
           ...
           DbEdit23.text := frmmodulo.qrclienteNome.asstring;
          end;
      end;
  end;
   ecfop.setfocus;
...
begin
  ...
  qrnotafiscal_item.sql.add('where item.codproduto = pro.codigo and');
  qrnotafiscal_item.sql.add('item.codnota = ''' +
   frmnotafiscal_menu.qrnota.fieldbyname('codigo').asstring + ''' order by item');
  qrnotafiscal_item.Open;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if qrnotafiscal_item.fieldbyname('codigo').asstring = '' then
        begin
          query.sql.add(',codigo = ''' + frmprincipal.codifica('000062','update') + '''');
...
procedure Tfrmnotafiscal.qrnotafiscal_item3NewRecord(DataSet: TDataSet);
begin
  ...
  begin
    ...
    if frmmodulo.qrcfop.RecordCount > 0 then
     begin
      ...
      if (resultado_pesquisa1 = '5104') or (resultado_pesquisa1 = '6104') or
            (resultado_pesquisa1 = '1904') or (resultado_pesquisa1 = '2904')
      then
        begin
         ...
         qry.insert ;
         frmnotafiscal_menu.qrnota.fieldbyname('INF4').asstring := 'NOTA N° '+FrmRemessa.NOTA+' EMITIDA NA DATA '+FrmRemessa.DATA;
         qry.fieldbyname('codigo').asstring := FrmPrincipal.codifica('nfe_complementar','ler');
         qry.fieldbyname('codnota').asstring := FrmPrincipal.codifica('000061','ler');
         qry.fieldbyname('numnota').asstring := edit5.Text;
         qry.fieldbyname('numnotaref').asstring := FrmRemessa.nota;
         qry.fieldbyname('data').asstring := FrmRemessa.data;
         qry.fieldbyname('chave').asstring := FrmRemessa.chave ;
         qry.post;
...
if ecfop.text <> '' then
   begin
        ...
        if frmmodulo.qrcfop.RecordCount > 0 then
          begin
            frmnotafiscal_menu.qrnota.fieldbyname('INF1').asstring := frmmodulo.qrcfop.fieldbyname('OBS3').asstring;
            frmnotafiscal_menu.qrnota.fieldbyname('INF2').asstring := frmmodulo.qrcfop.fieldbyname('OBS4').asstring;
            frmnotafiscal_menu.qrnota.fieldbyname('INF3').asstring := frmmodulo.qrcfop.fieldbyname('OBS3').asstring;
           // frmnotafiscal_menu.qrnota.fieldbyname('INF4').asstring := frmmodulo.qrcfop.fieldbyname('OBS4').asstring;
            frmnotafiscal_menu.qrnota.fieldbyname('cfop').asstring := frmmodulo.qrcfop.fieldbyname('cfop').asstring;
            edit5.setfocus;

            if (ecfop.text = '5104') or (ecfop.text = '6104') or
               (ecfop.text = '1904') or (ecfop.text = '2904') then
              begin
...
begin
  ...
  if frmnotafiscal_menu.qrnota.State = dsinsert then
    begin
      ...
      frmnotafiscal_menu.qrempresa.close;
      frmnotafiscal_menu.qrempresa.sql.clear;
      frmnotafiscal_menu.qrempresa.sql.add('select * from c000004 where codigo = ''' +
  ...
  if frmnotafiscal_menu.qrnota.State = dsinsert then
    Edit5.text := FrmModulo.codificanfe(SERIE,'55','ler')
  else
    Edit5.text := frmnotafiscal_menu.qrnota.fieldbyname('NUMERO').asstring;
  ...
  with frmmodulo do
    begin
     if natoperacao = 'venda' then
      begin
        rTipo.itemindex := 0;
        rTipo_Pagamento.itemindex := 0;
      end;
     if natoperacao = 'devolucao' then
      begin
        rTipo.itemindex := 0;
        rTipo_Pagamento.itemindex := 11;
      end;
     if natoperacao = 'garantia' then
      begin
        rTipo.itemindex := 0;
        rTipo_Pagamento.itemindex := 10;
      end;
    end;
   ecliente.setfocus;
end;
...
begin
  if qrreceber.State = dsinsert then
    qrreceber.fieldbyname('codigo').asstring := frmprincipal.codifica('000062','update');
