...
uses
  ...
  JvExMask, AdvSmoothButton, IPPeerClient, REST.Client, Data.Bind.Components,
...
uses
  ...
  cliente_inicio, xloc_cidade, webcam2, xloc_cnae,ucnpf, uCliente, REST.Json, REST.Types, pcnConversao ;
...
begin
  ...
  begin
    ...
    FrmModulo.qrcliente.fieldbyname('Codigo').asstring := FrmPrincipal.codifica('000007','ler');
...
begin
 ...
 //Valida Cep
  if DBEdit8.Text = '' then
   begin
    application.messagebox('Favor informar um CEP válido para este cliente!',
      'Atenção', mb_ok + mb_iconerror);
    DBEdit8.SetFocus;
    exit;
   end;
 ...
 //Valida RGCPF
   if DBEdit14.Text = '' then
    begin
       IF LRG.CAPTION = 'RG:' then
        application.messagebox('Favor informar um RG válido para este cliente!', 'Atenção', mb_ok + mb_iconerror)
       ELSE
        application.messagebox('Favor informar um I.E. válido para este cliente!', 'Atenção', mb_ok + mb_iconerror);
  ...
  begin
    FRMMODULO.qrcliente.FieldByName('codigo').asstring := FrmPrincipal.codifica('000007','update');
  ...
  if CONTINUA_INCLUSAO then
    ...
    frmprincipal.log(codigo_usuario, 'CLIENTE',FRMMODULO.qrcliente.FieldByName('codigo').asstring, 'INCLUIU',
                     FRMMODULO.qrcliente.FieldByName('NOME').asstring);
...
procedure Tfrmcliente.medtEditCNPJEnter(Sender: TObject);
begin
if medtEditCNPJ.text = 'Digite o CNPJ' then
  medtEditCNPJ.text := '';
end;

procedure Tfrmcliente.medtEditCNPJExit(Sender: TObject);
begin
    ...
    begin
      if pos('-', DBEdit12.Text) = 0 then
        FRMMODULO.qrcliente.FieldByName('CELULAR').asstring :=
          FormatMaskText('(99)99999-9999;0;_', DBEdit12.Text);
    end;
  end;
end;
...
procedure Tfrmcliente.advmtrbtnbtn1Click(Sender: TObject);
begin
      PnlCNPJ.Visible := False;
end;
...
begin
  ...
  try
      Cliente := TJson.JsonToObject<TClienteClass>(RESTResponse1.Content);
       lblsituacaoCNPJ.Text  := Cliente.situacao;

      frmmodulo.qrclienteNOME.AsString := Cliente.nome;

      frmmodulo.qrclienteENDERECO.AsString    := Cliente.logradouro;
      frmmodulo.qrclienteNUMERO.AsString      := Cliente.numero;
      frmmodulo.qrclienteCOMPLEMENTO.AsString := Cliente.complemento;
      frmmodulo.qrclienteBAIRRO.AsString      := Cliente.bairro;
      frmmodulo.qrclienteCIDADE.AsString      := Cliente.municipio;
      euf.Text          := Cliente.uf;
      frmmodulo.qrclienteCEP.AsString         := Cliente.cep;

      frmmodulo.qrclienteTELEFONE1.AsString       := Cliente.telefone;
      frmmodulo.qrclienteCPF.AsString      := Cliente.cnpj;
      edata_cadastro.Date := Date();
