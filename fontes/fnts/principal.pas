...
uses
  ...
  sSpeedButton, dxSkinsForm, ACBrBase, ACBrValidador,
  ...
  System.ImageList, SysUtils, AdvSmoothButton, ACBrDFe, ACBrNFe;
...
type
  ...
  TfrmPrincipal = class(TForm)
    ...
    bt_sintegra001: TAdvSmoothButton;
    lbl1: TLabel;
    AdvSmoothButton42: TAdvSmoothButton;
    img3: TImage;
    img4: TImage;
    advsmthbtn1: TAdvSmoothButton;
    advsmthbtnAction70: TAdvSmoothButton;
    Action32: TAction;
    advsmthbtnPed_Venda: TAdvSmoothButton;
    ...
    procedure advsmthbtn1Click(Sender: TObject);
    procedure AdvSmoothButton2MouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure AdvSmoothButton3MouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure AdvSmoothButton5MouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure AdvSmoothButton4MouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure AdvSmoothButton6MouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure imgFundoMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure advsmthbtnAction70Click(Sender: TObject);
    Procedure MapaRede(PServer, PSenha, PLetra : PChar);
  ...
  public
    ...
    function codifica(TABELA,ACTION: string): string;
    ...
    procedure BotaoCinza (Sender : TObject);
...
procedure AcertaPadraoData; External '\Store_Protheus\server\dll\ebo7.dll';

function NumeroSerieHD: string; External '\Store_Protheus\server\dll\ebo7.dll';
function frm(ebo: string): string; External '\Store_Protheus\server\dll\ebo7.dll';
...
var
  ...
  venda_TEF: Boolean;
  //
  bPn_ativo : boolean;  // Minimiza painel de atalho
  bPos_caixa : boolean; // Minimiza Painel caixa
  ...
  rProd_aliquota,rProd_Estoque,rProd_precovenda1,rProd_precovenda2,
  rProd_precovenda3,rProd_precovenda4,rProd_preco,rProd_qtde,rProd_total,
  ...
  bProd_promocao, sSair: boolean; // armazenar se o produto estah em promocao
  sProd_Tamanho: string;
  sProd_Cor,cNcm,cCfop: string;
...
uses modulo, senha, cliente,
  ...
  form_ativacaoicloud, pUCGeral, OpNatureza, Unit_ativar, UVendaCancelada,
  splash, produto_venda;
...
begin
  ...
  with frmmodulo do // Verifica se ha um caixa para o usuario logado
    begin
      qrusuario.close;
      qrusuario.SQL.Clear;
      qrusuario.sql.add('select a.* from c000003 a where a.usuario = '+label8.caption) ;
      qrusuario.open;
    end;
...
procedure TfrmPrincipal.advsmthbtn1Click(Sender: TObject);
begin
AdvSmoothPanel5.Visible := true;
end;

procedure TfrmPrincipal.advsmthbtnAction70Click(Sender: TObject);
begin
...
begin
  ...
  begin
    ...
    frmmodulo.queryagenda.SQL.add('(:codigo,:tipo,:nome,:telefone1,:telefone2,:telefone3,:celular,:email,:fax,:homepage,:tipoi)');
    frmmodulo.queryagenda.params.parambyname('codigo').asstring := frmPrincipal.codifica('000034','update');
...
begin
  ...
  frmproduto_venda := tfrmproduto_venda.create(self);
  frmproduto_venda.showmodal;
end;
...
function TfrmPrincipal.codifica(TABELA,ACTION : string): string;
var
  xcod: Integer;
begin
  qrmestre.close;
  qrmestre.SQL.clear;
  qrmestre.SQL.add('select * from c000000 where codigo = ''' + TABELA + ''' WITH LOCK');

  try
   qrmestre.open;
  except
    showmessage('N„o foi possÌvel encontrar a tabela desejada !');
  end;
  ...
  if qrmestre.recordcount >= 0 then
    begin
      ...
      if action = 'update' then
       ...
       result := frmPrincipal.zerarcodigo(IntToStr(xcod+1), 6);
...
begin
 ...
 //informaÁıes de ativaÁ„o do sistema
  ...
  if (frmmodulo.Conexao.HostName <> '') and
    ...
    and (frmmodulo.Conexao.HostName <> GetIP)
  then
    begin
     // showmessage('sistema eh uma estacao');

    // MapaRede('\\maquinanarede\pastacompartilhada' + #0,'Z:','');
    end
  else
    begin
 ...
 if USA_TEF then
  ...
  try
    ...
    if not Registro.OpenKey('Store_Protheus', false) then
    begin
      Registro.CreateKey('Store_Protheus');
      Registro.OpenKey('Store_Protheus', true);
...
begin
  // store_protect.IDSistema := 2021;
  // store_protect.executar;
  sPath_exec := extractfilepath(paramstr(0));
  sPath_ini  := extractfilepath(paramstr(0)) +'ini\';

  try

    assignfile(txt, sPath_ini + 'com.ini');
    reset(txt);

     while not eof(txt) do
      begin
       readln(txt, entrada);
       if copy(entrada, 1, 1) <> '*' then
        begin
        if copy(entrada, 1, 7) = '999-002' then
         conexao_sistema := trim(copy(entrada, 9, 50));
        if copy(entrada, 1,5) = 'SERIE' then
          SERIE := trim(copy(entrada,7 ,3));

        end;

      end;

    bPos_caixa := true; // Painel de Caixa Maximizado
    bPn_ativo := true; // Painel de Atalho Maximizado
    closefile(txt);
    ...
    ExeAtualiza := ExtractFilePath(ParamStr(0)) + 'atualizacao\Store_Protheus.exe';
      ...
      begin
        ...
        begin
          ...
          CopyFile(Pchar(ExeAtualiza),
            Pchar(extractfilepath(paramstr(0)) +'Store_Protheus.exe'), false);
    ...
    caption := caption + '    Terminal: ' + registro_terminal;
     ...
     //fim
      ...
      label17.top := (AdvSmoothExpanderPanel1.Height div 2)-(label17.height div 2);
     // lbl1.top   := (AdvSmoothExpanderPanel1.height div 2) -(label17.height div 2);
...
begin
    ...
    frmEMPRESA.EdtSerie.text := SERIE;
          ...
          begin
            if frmmodulo.qrcaixa_operador.FIELDBYNAME('situacao').AsInteger = 1 then
             ...
             begin
              application.messagebox('Este caixa est· fechado! Favor verificar...',
                'AtenÁ„o', mb_ok + mb_iconwarning);
                AdvSmoothButton32.click;
              Exit;
             end;
          end
        else
        if frmmodulo.qrcaixa_operador.FIELDBYNAME('situacao').AsInteger = 1 then // Caixa situaÁ„o = 1 aberto
          begin
           frmVENDA_INICIO := tfrmVENDA_INICIO.create(self);
           frmVENDA_INICIO.showmodal;
          end
          ...
          begin
            if Application.messageBox('Este caixa est· fechado, Deseja verificar ?','ConfirmaÁ„o',mb_YesNo+mb_IconInformation+mb_DefButton2) = IDYES then
              begin
               if frmPrincipal.autentica_caixa('Acesso ao Caixa', 0) then
                 begin
                   frmcaixa := tfrmcaixa.create(self);
...
begin
  WinExec('C:\Store_Protheus\ferramentas\suporte.exe', SW_SHOW);
end;
...
begin
  barra_cadastros.Visible:= False;
 // showmessage(frmModulo.conexao.hostname +#13+frmModulo.conexao.database );
  try
...
procedure TfrmPrincipal.BotaoCinza(Sender: TObject);
begin
    AdvSmoothButton1.color := $00363636;
    AdvSmoothButton2.color := $00363636;
    AdvSmoothButton3.color := $00363636;
    AdvSmoothButton4.color := $00363636;
    AdvSmoothButton5.color := $00363636;
    AdvSmoothButton6.color := $00363636;
...
procedure TfrmPrincipal.AdvSmoothButton2Click(Sender: TObject);
begin
...
procedure TfrmPrincipal.AdvSmoothButton2MouseMove(Sender: TObject;
  Shift: TShiftState; X, Y: Integer);
begin
BotaoCinza(sender);
AdvSmoothButton2.Color := $0043A352 ;
end;
...
procedure TfrmPrincipal.AdvSmoothButton3Click(Sender: TObject);
begin
...
procedure TfrmPrincipal.AdvSmoothButton3MouseMove(Sender: TObject;
  Shift: TShiftState; X, Y: Integer);
begin
BotaoCinza(Sender);
AdvSmoothButton3.Color := $00BA843A;
end;
...
procedure TfrmPrincipal.AdvSmoothButton42Click(Sender: TObject);
begin
 barra_movimento.Visible:= False;
 FrmVCancelada := tFrmVCancelada.Create(Self);
 FrmVCancelada.Showmodal;
end;

procedure TfrmPrincipal.AdvSmoothButton4Click(Sender: TObject);
begin
...
procedure TfrmPrincipal.AdvSmoothButton4MouseMove(Sender: TObject;
  Shift: TShiftState; X, Y: Integer);
begin
botaocinza(Sender);
AdvSmoothButton4.color := $000080FF;
end;

procedure TfrmPrincipal.AdvSmoothButton5Click(Sender: TObject);
begin
...
procedure TfrmPrincipal.AdvSmoothButton5MouseMove(Sender: TObject;
  Shift: TShiftState; X, Y: Integer);
begin
BotaoCinza(Sender);
AdvSmoothButton5.color := $00FF0080;
end;

procedure TfrmPrincipal.AdvSmoothButton6Click(Sender: TObject);
begin
...
procedure TfrmPrincipal.AdvSmoothButton6MouseMove(Sender: TObject;
  Shift: TShiftState; X, Y: Integer);
begin
botaocinza(Sender);
AdvSmoothButton6.color := cLOLIVE;
end;
...
procedure TfrmPrincipal.imgFundoMouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
begin
botaoCinza(sender);
end;

procedure TfrmPrincipal.img3Click(Sender: TObject);
var
pn_top : integer;
...
begin
  if bPn_ativo  then
     begin
      pn_top := AdvSmoothPanel15.top;
      AdvSmoothPanel15.top := AdvSmoothPanel2.top - img3.height;
      bPn_ativo := false ;

     end
  else
    begin
     AdvSmoothPanel15.top := frmPrincipal.height - AdvSmoothPanel15.height - AdvSmoothPanel2.height - 5;
     bPn_ativo := true;
    end;
...
begin
  if bPos_caixa  then
     begin
      st_caixa := barra_caixa.top;
      barra_caixa.top := AdvSmoothPanel15.height - img4.height;
      bPos_caixa := false ;
     end
  else
  begin
   barra_caixa.top := AdvSmoothPanel15.height - barra_caixa.height - 5;
   bPos_caixa := true;
  end;
...
procedure TfrmPrincipal.MapaRede(PServer, PSenha, PLetra : PChar);
var
  err : DWord;
Begin
  ...
  case ERR of
    ...
    ERROR_BAD_DEV_TYPE:              ShowMessage ( 'O tipo de dispositivo e o tipo de recurso n„o s„o compatÌveis.' );
    ERROR_BAD_DEVICE:                ShowMessage ( 'Letra inv·lida.' );
    ERROR_BAD_NET_NAME:              ShowMessage ( 'Nome do servidor n„o È v·lido ou n„o pode ser localizado.' );
    ERROR_BAD_PROFILE:               ShowMessage ( 'Formato incorreto de par‚metros.' );
    ERROR_CANNOT_OPEN_PROFILE:       ShowMessage ( 'Conex„o permanente n„o disponÌvel.' );
    ...
    ERROR_NO_NET_OR_BAD_PATH:        ShowMessage ( 'A operaÁ„o n„o foi concluÌda porque a rede n„o foi inicializada ou caminho È inv·lido.' );
    ERROR_NO_NETWORK:                ShowMessage ( 'A rede n„o est· presente.' );
  else
      ShowMessage ('Erro n„o especificardo');
  end;
end;
