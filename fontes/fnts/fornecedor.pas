...
uses
  ...
  JvDBControls, IPPeerClient, REST.Client, Data.Bind.Components,
...
type
  Tfrmfornecedor = class(TForm)
    ...
    dtfldQRNF_PRODUTODATA: TDateField;
    ...
    dtfldQRNF_PRODUTOLOTE_VALIDADE: TDateField;
    dtfldQRNF_PRODUTOLOTE_FABRICACAO_DATA: TDateField;
    ...
    adpnl1advsmthxpndrpnl1: TAdvSmoothExpanderPanel;
    lbl3: TLabel;
    advmtrbtnbtn1: TAdvMetroButton;
    ...
    procedure medtEditCNPJEnter(Sender: TObject);
    procedure medtEditCNPJExit(Sender: TObject);
    procedure advmtrbtnbtn1Click(Sender: TObject);
...
uses modulo, principal, loc_regiao, regiao, loc_funci, xloc_fornecedor,
  lista_fornecedor2, fornecedor_compras, xloc_cidade, ucnpf, uCliente, REST.Json, REST.Types, pcnConversao ;
...
begin
  CONTINUA_INCLUSAO := true;
    ...
    frmmodulo.qrFORNECEDOR.fieldbyname('codigo').asstring := frmprincipal.codifica('000009','ler');
    frmmodulo.qrFORNECEDOR.fieldbyname('assistencia_tecnica').asinteger := 0;
 //  DBedit1.text := frmprincipal.codifica('000009','ler');
 // DBCheckBox1.itemIndex := 0 ;
...
procedure Tfrmfornecedor.advmtrbtnbtn1Click(Sender: TObject);
begin
...
begin
  if (medtEditCNPJ.text = '') or (medtEditCNPJ.text = 'Digite o CNPJ') then
     exit ;
  if length(SoNumero(medtEditCNPJ.text)) <> 14 then
     begin
     showmessage('ATENÇÃO O CNPJ DEVE ATENDER O PADRÃO DE 14 DIGITOS SEM (.)pontos ou (/)barra') ;
     exit;
     end;

  RESTRequest1.Method := rmGet;
  RESTClient1.BaseURL := 'https://www.receitaws.com.br/v1/cnpj/';
  RESTRequest1.Resource := SoNumero(medtEditCNPJ.Text );
  RESTRequest1.Execute;

  fornecedor := TClienteClass.Create;


  try
      fornecedor := TJson.JsonToObject<TClienteClass>(RESTResponse1.Content);
       lblsituacaoCNPJ.Text  := fornecedor.situacao;
      frmmodulo.qrFORNECEDORNOME.AsString := fornecedor.nome;
      frmmodulo.qrFORNECEDORENDERECO.AsString    := fornecedor.logradouro;
      frmmodulo.qrFORNECEDORNUMERO.AsString      := fornecedor.numero;
      frmmodulo.qrFORNECEDORCOMPLEMENTO.AsString := fornecedor.complemento;
      frmmodulo.qrFORNECEDORBAIRRO.AsString      := fornecedor.bairro;
      frmmodulo.qrFORNECEDORCIDADE.AsString      := fornecedor.municipio;
      euf.Text          := fornecedor.uf;
      frmmodulo.qrFORNECEDORCEP.AsString         := fornecedor.cep;

      frmmodulo.qrFORNECEDORTELEFONE1.AsString       := fornecedor.telefone;
      frmmodulo.qrFORNECEDORCNPJ.AsString      := fornecedor.cnpj;
      edata_cadastro.Date := Date();
      frmmodulo.qrFORNECEDORIE.AsString   := 'ISENTO';
   //   frmmodulo.qrFORNECEDORIE.AsString := fornecedor.efr;

   //   DateEdit1.Date := strtodate(Cliente.data_situacao);
      fornecedor.Free;
      medtEditCNPJ.text := 'Digite o CNPJ';
      pnlcnpj.visible := false;

  except
      showmessage('Não Foi Possivel encontrar o CNPJ informado !');
  end;
...
begin
  ...
  begin
    ...
    if CLI_PESSOA = 'FÍSICA' then
     frmmodulo.qrFORNECEDOR.fieldbyname('TIPO').asinteger := 1
    else
     frmmodulo.qrFORNECEDOR.fieldbyname('TIPO').asinteger := 2;
...
procedure Tfrmfornecedor.medtEditCNPJEnter(Sender: TObject);
begin
if medtEditCNPJ.text = 'Digite o CNPJ' then
  medtEditCNPJ.text := '';
end;

procedure Tfrmfornecedor.medtEditCNPJExit(Sender: TObject);
begin
 if medtEditCNPJ.text = '' then
  medtEditCNPJ.text := 'Digite o CNPJ';
end;
...
procedure Tfrmfornecedor.dsfornecedor2DataChange(Sender: TObject;
  Field: TField);
begin
  if frmmodulo.qrfornecedor.fieldbyname('tipo').value = 1 then
   begin
    //  combo_pessoa.SetFocus;
      combo_pessoa.ItemIndex := 0;
      Label33.Caption := 'Nome:';
