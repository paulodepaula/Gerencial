...
//              ShowMessage(qrproduto.fieldbyname('codbarra').asstring);
      ...
      if FrmNotafiscal.edit3.text = FrmNotafiscal.edit4.text then
       ed_cfop.Text := qrproduto.fieldbyname('IND_CFOP_VENDA_DENTRO').asstring
      else
       ed_cfop.Text := qrproduto.fieldbyname('IND_CFOP_VENDA_FORA').asstring;
...
procedure TfrmNotaFiscal_Item2.tmr1Timer(Sender: TObject);
var
tipomov : string;
begin
// Completa descrição CST
    if ed_cst.Text <> '' then
     begin
      query1.close;
      query1.sql.clear;
...
// Completa Descrição CFOP
  if ed_cfop.Text <> '' then
    begin
     if frmnotafiscal.combomovimento.itemIndex = 0 then
       tipomov := '2'
       else tipomov := '1';

      query1.close;
      query1.sql.clear;
      query1.sql.add('select * from c000030 where cfop = '''+ed_cfop.text+''' and tipo ='+tipomov);
      query1.open;
      if query1.recordcount = 1 then
       begin
...
begin
  ...
  if ed_classific.text = '' then
  begin
    application.messagebox('Favor verifcar o Código NCM no cadastro do Produto!', 'Erro', mb_ok + mb_iconerror);
    ed_classific.setfocus;
    exit;
  end;
  if ed_cfop.text = '' then
  begin
  ...
  begin
    ...
    FrmNotaFiscal.qrnotafiscal_item.Params.Parambyname('CODLANCAMENTO').asstring := frmprincipal.codifica('000032','update');
    ...
    frmNotaFiscal.qrnotafiscal_item.Params.Parambyname('codigo').asstring := frmprincipal.codifica('000062','update');
...
begin
  ...
  begin
    ...
    begin
      frmmodulo.qrcfop.close;
      frmmodulo.qrcfop.sql.clear;
      frmmodulo.qrcfop.sql.add('select * from c000030 where cfop = '''+ed_cfop.text+''' and tipo = 1');
      frmmodulo.qrcfop.open;
...
procedure TfrmNotaFiscal_Item2.ed_unitarioKeyPress(Sender: TObject;
  var Key: Char);
begin
 if ed_cst.text = '' then
  ed_cst.setfocus;
 ...
 if (ed_cst.text <> '') and  (ed_cfop.text <> '') and (ed_csosn.text <> '') then
   bgravar.setfocus;
