...
uses
  ...
  AdvGlowButton, DBAccess, RzEdit, RzSpnEdt, frxDesgn, frxClass, frxDBSet, ACBrBase,ACBrBoleto, AdvMetroButton, AdvSmoothPanel,
  AdvSmoothExpanderPanel, JvMemoryDataset, JvExMask, JvToolEdit, JvBaseEdits;

type
  Tfrmvenda_fechamento2 = class(TForm)
...
var
  frmvenda_fechamento2: Tfrmvenda_fechamento2;
...
uses venda, modulo, principal, venda_contasreceber, troco, venda_impressao,
  ...
  cartao_ficha, OpNatureza, UFuncoes, venda_fechamento, formas_pgto;

{$R *.dfm}

function Tfrmvenda_fechamento2.retorna_letra(numero: Integer): string;
begin
  case numero of
    1:
      result := 'A';
...
procedure Tfrmvenda_fechamento2.rdesconto1KeyPress(Sender: TObject;
  var Key: Char);
begin
  if Key = #13 then
...
procedure Tfrmvenda_fechamento2.racrescimo2KeyPress(Sender: TObject;
  var Key: Char);
begin
  if Key = #13 then
    rdinheiro.SetFocus;
...
procedure Tfrmvenda_fechamento2.rdesconto1Enter(Sender: TObject);
begin
  TEdit(Sender).color := $00A8FFFF;;
end;

procedure Tfrmvenda_fechamento2.rdesconto1Exit(Sender: TObject);
begin
...
procedure Tfrmvenda_fechamento2.rdesconto2Exit(Sender: TObject);
begin
  TEdit(Sender).color := clwindow;
...
procedure Tfrmvenda_fechamento2.racrescimo1Exit(Sender: TObject);
begin
  TEdit(Sender).color := clwindow;
...
procedure Tfrmvenda_fechamento2.racrescimo2Exit(Sender: TObject);
begin
  TEdit(Sender).color := clwindow;
  if rsubtotal.value <> 0 then
  begin
...
procedure Tfrmvenda_fechamento2.rdinheiroExit(Sender: TObject);
begin
  TEdit(Sender).color := clwindow;
...
procedure Tfrmvenda_fechamento2.bgravarClick(Sender: TObject);
var
  texto: PWideChar;
  ...
  d, m, a: word;
  Titulo : TACBrTitulo;
  // variaveis das parcela
  prestacao,iTipoPgto: Integer;
  valor: real;
  StrX: string;
  rsoma2: real;
...
begin
  ...
  if (rdinheiro.Value + rchequeav.Value + rchequeap.Value+ rcartaodeb.Value + rcartaocred.Value +
      rconvenio.Value + rcrediario.Value + rfinanceira.Value + rsoma.Value + rdiferenca.Value + rboleto) > 0
  then
    begin
      //  if frmmodulo.qrcondpgto_parcela.RecordCount > 0 then
          ...
          if rdinheiro.Value > 0 then // Inseri pagamento em dinheiro
           begin
            frmvenda.qrvenda_contasreceber.Insert;
            frmvenda.qrvenda_contasreceber.fieldbyname('prestacao').AsInteger := 0;
           ...
           frmprincipal.codifica('000048','update');
           frmvenda.qrvenda_contasreceber.open;
  ...
  if rdiferenca.Value <> 0 then
   begin
    application.messagebox
      ('Os valores informados nas parcelas não confere com o valor total desta venda! Favor verificar!', 'Atenção', mb_ok + MB_ICONWARNING);
 ...
 if frmmodulo.natoperacao = 'venda' then
   begin
    if (rdinheiro.value = 0.00) and (rchequeav.value = 0.00) and
       ...
       (rsoma.value = 0.00) and (rdiferenca.value = 0.00)
    then
     begin
       Exit;
     end;
  ...
  try
    ...
    begin
      ...
      qrcaixa_mov.parambyname('codigo').asstring := frmprincipal.codifica('000044','update');
    ...
    begin
      ...
      qrcaixa_mov.parambyname('codigo').asstring :=
        frmprincipal.codifica('000044','update');
  ...
  //  frmvenda.qrvenda_contasreceber.First;
   ...
   //   showmessage(frmvenda.qrvenda_contasreceber.sql.text);
      ...
      begin
        ...
        FI_LANCTO := frmprincipal.codifica('000123','update');
        ...
        begin
          ...
          begin
            ...
            IBQuery1.sql.add
              (':liquido, :data_recebimento, :situacao, :tipo, :parcela, :valor_total)');
            IBQuery1.parambyname('codigo').asstring :=
              frmprincipal.codifica('000124','update');
    ...
    frmvenda_impressao.iTipoPgto := iTipoPgto;
  ...
  //  frmmodulo.boleto.Imprimir;
    ...
    begin
      ...
      IBQuery1.params.parambyname('xcodigo').asstring := frmprincipal.codifica('000032','update');
      ...
      begin
        ...
        begin
          ...
          qrcontasreceber.sql.add('select * from c000049 where codvenda = ''' +
            frmvenda.lvenda_codigo.caption + ''' and situacao = 1 order by data_vencimento');
          qrcontasreceber.Open;
          if qrcontasreceber.RecordCount > 0 then
            begin
              frmmodulo.qrrelatorio.Open;
              frmmodulo.qrrelatorio.edit;
              frmmodulo.qrrelatorio.FieldByName('linha1').asstring := emitente_cidade + FormatDateTime
                ('", "dddd", "d" de "mmmm" de "yyyy',frmmodulo.qrvenda.FieldByName('data').asdatetime);
              frmmodulo.qrrelatorio.FieldByName('linha2').asstring := AnsiUpperCase(extenso(frmvenda_fechamento2.rtotal.value));
...
procedure Tfrmvenda_fechamento2.bcancelarClick(Sender: TObject);
begin
  close;
end;

procedure Tfrmvenda_fechamento2.FormShow(Sender: TObject);
begin
...
procedure Tfrmvenda_fechamento2.rdinheiroEnter(Sender: TObject);
begin
  TEdit(Sender).color := $00A8FFFF;;
  edinheiro.Brush.color := $00A8FFFF;;
end;

procedure Tfrmvenda_fechamento2.rchequeavEnter(Sender: TObject);
begin
  TEdit(Sender).color := $00A8FFFF;;
  echequeav.Brush.color := $00A8FFFF;;
end;

procedure Tfrmvenda_fechamento2.rchequeapEnter(Sender: TObject);
begin
  TEdit(Sender).color := $00A8FFFF;;
  echequeap.Brush.color := $00A8FFFF;;
end;

procedure Tfrmvenda_fechamento2.rcartaocredEnter(Sender: TObject);
begin
  TEdit(Sender).color := $00A8FFFF;;
  ecartaocred.Brush.color := $00A8FFFF;;
end;

procedure Tfrmvenda_fechamento2.rcartaodebEnter(Sender: TObject);
begin
  TEdit(Sender).color := $00A8FFFF;;
  ecartaodeb.Brush.color := $00A8FFFF;;
end;

procedure Tfrmvenda_fechamento2.rcrediarioEnter(Sender: TObject);
begin
  TEdit(Sender).color := $00A8FFFF;;
  ecrediario.Brush.color := $00A8FFFF;;
end;

procedure Tfrmvenda_fechamento2.rcrediarioKeyPress(Sender: TObject;
  var Key: Char);
begin
  if Key = #13 then
    bgravar.SetFocus;
...
procedure Tfrmvenda_fechamento2.Cancelar1Click(Sender: TObject);
begin
  close;
end;

procedure Tfrmvenda_fechamento2.rchequeavExit(Sender: TObject);
begin
  TEdit(Sender).color := clwindow;
...
procedure Tfrmvenda_fechamento2.rchequeapExit(Sender: TObject);
begin
  TEdit(Sender).color := clwindow;
...
procedure Tfrmvenda_fechamento2.rcartaocredExit(Sender: TObject);
begin
  TEdit(Sender).color := clwindow;
...
procedure Tfrmvenda_fechamento2.rcartaodebExit(Sender: TObject);
begin
  TEdit(Sender).color := clwindow;
...
procedure Tfrmvenda_fechamento2.rcrediarioExit(Sender: TObject);
begin
  TEdit(Sender).color := clwindow;
...
procedure Tfrmvenda_fechamento2.BitBtn1Click(Sender: TObject);
begin
...
procedure Tfrmvenda_fechamento2.rconvenioEnter(Sender: TObject);
begin
  TEdit(Sender).color := $00A8FFFF;;
...
procedure Tfrmvenda_fechamento2.rconvenioExit(Sender: TObject);
begin
  TEdit(Sender).color := clwindow;
  ...
  begin
    ...
    begin
      ...
      if frmvenda_fechamento2.rdesconto1.value > 0 then
      begin
      ...
      begin
        rconvenio_liquido.value := frmvenda_fechamento2.rsubtotal.value -
          (frmvenda_fechamento2.rsubtotal.value *
          ((rconvenio_desconto.value + frmvenda_fechamento2.rdesconto1.
          value) / 100));
      end
      else
        rconvenio_liquido.value := frmvenda_fechamento2.rtotal.value;
    end;
  end;
...
procedure Tfrmvenda_fechamento2.rconvenioKeyPress(Sender: TObject;
  var Key: Char);
begin
  if Key = #13 then
    perform(wm_nextdlgctl, 0, 0);
...
procedure Tfrmvenda_fechamento2.econvenio_codclienteEnter(Sender: TObject);
begin
  TEdit(Sender).color := $00A8FFFF;;

end;

procedure Tfrmvenda_fechamento2.econvenio_codclienteExit(Sender: TObject);
begin
  TEdit(Sender).color := clwindow;
...
procedure Tfrmvenda_fechamento2.econvenio_codclienteKeyPress(Sender: TObject;
  var Key: Char);
begin
  if Key = #13 then
    perform(wm_nextdlgctl, 0, 0);
...
procedure Tfrmvenda_fechamento2.bconvenio_clienteClick(Sender: TObject);
begin
...
procedure Tfrmvenda_fechamento2.convenio_comboExit(Sender: TObject);
begin
  pconvenio.Visible := TRUE;
end;

procedure Tfrmvenda_fechamento2.Button1Click(Sender: TObject);
var
  F: TEXTFILE;
...
procedure Tfrmvenda_fechamento2.FormClose(Sender: TObject;
...
procedure Tfrmvenda_fechamento2.rfinanceiraEnter(Sender: TObject);
begin
  TEdit(Sender).color := $00A8FFFF;;
  efinanceira.Brush.color := $00A8FFFF;;
end;

procedure Tfrmvenda_fechamento2.rfinanceiraExit(Sender: TObject);
begin
  TEdit(Sender).color := clwindow;
...
procedure Tfrmvenda_fechamento2.rfinanceiraKeyPress(Sender: TObject;
  var Key: Char);
begin
  if Key = #13 then
    bgravar.SetFocus;
...
procedure Tfrmvenda_fechamento2.BCH_TECBANClick(Sender: TObject);
var
  vlr: string;
  i: Integer;
  F: TEXTFILE;
...
procedure Tfrmvenda_fechamento2.BCH_REDECARDClick(Sender: TObject);
var
  vlr: string;
  i: Integer;
  F: TEXTFILE;
...
procedure Tfrmvenda_fechamento2.bcartaoClick(Sender: TObject);
var
  i: Integer;
  F: TEXTFILE;
  vlr: string;
...
procedure Tfrmvenda_fechamento2.bpgtoClick(Sender: TObject);
var
  ret: Integer;
  ecf_ok: boolean;
begin
...
procedure Tfrmvenda_fechamento2.cb_creditoKeyPress(Sender: TObject;
  var Key: Char);
begin
  if Key = #13 then
...
procedure Tfrmvenda_fechamento2.spin_creditoKeyPress(Sender: TObject;
  var Key: Char);
begin
  if Key = #13 then
    perform(wm_nextdlgctl, 0, 0);
...
procedure Tfrmvenda_fechamento2.cb_debitoKeyPress(Sender: TObject;
  var Key: Char);
begin
  if Key = #13 then
...
procedure Tfrmvenda_fechamento2.spin_debitoKeyPress(Sender: TObject;
  var Key: Char);
begin
  if Key = #13 then
    perform(wm_nextdlgctl, 0, 0);
...
procedure Tfrmvenda_fechamento2.Gravar1Click(Sender: TObject);
begin
  bgravarClick(frmvenda_fechamento2);
end;

procedure Tfrmvenda_fechamento2.bduplicataClick(Sender: TObject);
var
  sano: string;
  sOrdem: string;
  d, m, a: word;
