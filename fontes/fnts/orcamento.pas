...
uses
  ...
  Menus, RzEdit, RzDBEdit, RzDBBnEd, AdvShapeButton, AdvGlowButton, DBAccess, AdvGroupBox, frxClass, frxDBSet, frxDesgn, cxGraphics,
  ...
  // units nfe acbr
  pcnConversao, ACBrUtil,// SHDocVw,
  ComCtrls, FileCtrl,
  inifiles, IdMessage, IdBaseComponent, IdComponent,
  IdTCPConnection, IdTCPClient, IdMessageClient, IdSMTP, IdAttachment, IdAuthentication, IdAttachmentFile, AdvMetroButton,
...
type
  TfrmOrcamento = class(TForm)
    ...
    procedure edtSOLICITANTEEnter(Sender: TObject);
...
uses modulo, loc_cliente, principal, loc_funci, Math, orcamento_impressao,
  ...
  historico, cliente, produto, fornecedor, config, orcamento_grade, venda_serial, orcamento_serial,
  os_item_3, ecf;
...
begin
   ...
   codigo :=  frmprincipal.codifica('000056','ler');
   frmmodulo.qrorcamento.Last;
   ...
   if codigo <> '' then
    begin
     ...
     qrorcamento_contasreceber.Close;
     qrorcamento_contasreceber.sql.Clear;
     qrorcamento_contasreceber.sql.Add('select * from c000060 where codorcamento = ''' + codigo +
      ''' order by prestacao');
     qrorcamento_contasreceber.Open;
     qrClientesOrc.Open;
    end;
...
begin
  ...
  if parcinvalida > 0 then
   begin
    application.messagebox(Pchar('Existe ' + InttoStr(parcinvalida) +
      ' Condições de Pagamento Inválida!'), 'Atenção!', mb_ok + MB_ICONEXCLAMATION);
    exit;
   end;
  ...
  begin
      ...
      if application.messagebox('Deseja imprimir este orçamento?', 'Aviso', mb_yesno + MB_ICONQUESTION) = idyes then
        begin
          frmmodulo.qrcliente.locate('codigo',
            frmmodulo.qrorcamento.fieldbyname('codcliente').asstring, [loCaseInsensitive]);
    ...
    frmprincipal.codifica('000056','update');
...
procedure TfrmOrcamento.edtSOLICITANTEEnter(Sender: TObject);
begin
frmmodulo.qrorcamento.edit;
end;
          ...
          begin
            ...
            begin
              ...
              frmmodulo.qrlotelocal.params.ParamByName('xcodigo').asstring :=
                frmprincipal.codifica('000130','update');
...
procedure TfrmOrcamento.btn1Click(Sender: TObject);
begin
//frmmodulo.qrorcamento.edit;
 if frmmodulo.qrorcamento.state = dsinsert then
  showmessage('QrOrçamento esta modo Insert.');
 if frmmodulo.qrorcamento.state = dsEdit then
  showmessage('QrOrçamento esta modo Edite .');
 edtSOLICITANTE.enabled := true;
// frmmodulo.qrorcamento.fieldbyname('solicitante').asString := InputBox('Solicitante','Informe um solicitante', '') ;
//frmmodulo.qrorcamento.post;
end;
