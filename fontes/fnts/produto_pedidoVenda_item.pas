...
uses
  ...
  RzBtnEdt, JvExMask, JvToolEdit;

type
  Tfrmproduto_pedidoVenda_item = class(TForm)
    pgravar: TFlatPanel;
    Pop2: TPopupMenu;
    Gravar1: TMenuItem;
    Cancelar1: TMenuItem;
...
var
  frmproduto_pedidoVenda_item: Tfrmproduto_pedidoVenda_item;
  qtde_anterior, valor_anterior: real;
  vsugestao: integer;
implementation

uses produto_pedido, modulo, principal, produto_grade_pedido, produto_venda,
  xloc_produto, produto_pedido_item;

{$R *.dfm}

procedure Tfrmproduto_pedidoVenda_item.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure Tfrmproduto_pedidoVenda_item.bcancelarClick(Sender: TObject);
begin
  if dspedido_item.State = dsinsert then
  begin
    qrgrade_pedido.Close;
    qrgrade_pedido.SQL.clear;
    qrgrade_pedido.SQL.add('select * from c000143 where coditem = ''' + dspedido_item.dataset.fieldbyname('codigo').asstring + '''');
    qrgrade_pedido.Open;
...
procedure Tfrmproduto_pedidoVenda_item.eprodutoKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key = #13 then perform(wm_nextdlgctl, 0, 0);
end;

procedure Tfrmproduto_pedidoVenda_item.DBEdit5Enter(Sender: TObject);
begin
  TEdit(sender).Color := $00A0FAF8;
end;

procedure Tfrmproduto_pedidoVenda_item.DBEdit5Exit(Sender: TObject);
begin
...
procedure Tfrmproduto_pedidoVenda_item.DBEdit2Exit(Sender: TObject);
begin
  TEdit(sender).Color := clwindow;
  if (dspedido_item.state = dsinsert) or (dspedido_item.State = dsedit) then
  begin
...
procedure Tfrmproduto_pedidoVenda_item.FormShow(Sender: TObject);
begin
 // frmmodulo.
...
procedure Tfrmproduto_pedidoVenda_item.eprodutoExit(Sender: TObject);
begin
...
procedure Tfrmproduto_pedidoVenda_item.bgravarClick(Sender: TObject);
var st: integer;
begin
  ...
  frmproduto_pedido.qrpedido_item.fieldbyname('codigo').asstring := frmprincipal.codifica('000142','update');
...
procedure Tfrmproduto_pedidoVenda_item.DBEdit3Exit(Sender: TObject);
begin
  TEdit(sender).Color := clwindow;
  if (dspedido_item.state = dsinsert) or (dspedido_item.State = dsedit) then
  begin
...
procedure Tfrmproduto_pedidoVenda_item.bgradeClick(Sender: TObject);
begin
...
procedure Tfrmproduto_pedidoVenda_item.DBEdit4KeyPress(Sender: TObject;
  var Key: Char);
begin
  if key = #13 then bgravar.setfocus;
end;

procedure Tfrmproduto_pedidoVenda_item.DBEdit5KeyPress(Sender: TObject;
  var Key: Char);
begin
  if key = #13 then BGRAVAR.SETFOCUS;
end;

procedure Tfrmproduto_pedidoVenda_item.DBEdit3KeyPress(Sender: TObject;
  var Key: Char);
begin
  if key = #13 then
  begin
...
procedure Tfrmproduto_pedidoVenda_item.qrsugestaoCalcFields(DataSet: TDataSet);
begin
  QUERY.close;
  QUERY.sql.clear;
...
procedure Tfrmproduto_pedidoVenda_item.GRID5DblClick(Sender: TObject);
begin
...
procedure Tfrmproduto_pedidoVenda_item.ldataini_remarcadoExit(Sender: TObject);
begin
...
procedure Tfrmproduto_pedidoVenda_item.GRID5KeyPress(Sender: TObject;
  var Key: Char);
begin
  if key = #13 then GRID5DblClick(frmproduto_pedido_item);
end;

procedure Tfrmproduto_pedidoVenda_item.eprodutoButtonClick(
  Sender: TObject);
begin
  resultado_pesquisa1 := '';
