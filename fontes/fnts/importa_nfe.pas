...
uses ACBrNFeNotasFiscais, pcnNFe, MaskUtils, loc_conta, modulo, principal,
  Produto, UFuncoes, status,math,UFsist ;
...
begin
  ...
  FrmFsist := TFrmFsist.create(self);
  uFsist.sChave := eid.text;
 ...
 // OpenDialog1.InitialDir := ACBrNFe1.Configuracoes.Arquivos.PathSalvar ;
  OpenDialog1.InitialDir := '%Downloads%';
   Destino := extractfilepath(paramstr(0))+'DFe\XML\NfeEnt\'+FormatDateTime('MM-yyyy',date);
  ...
  if OpenDialog1.Execute() then
   begin
   //   if eid.text <> ''then
   //    OpenDialog1.FileName := '%Downloads%'+eid.text+'.xml';
  ...
  // OpenDialog1.FileName := ACBrNFe1.Configuracoes.Geral.PathSalvar+'\'+eid.Text;
    ...
    qrnota.Insert;
    qrnotaCODIGO.AsString := frmprincipal.codifica('000087','update');
    ...
    begin
      ...
      NomeProduto := '';
      vcodBarra_produto := '';
      vcod_item := frmprincipal.codifica('000032','update');
      ...
      //NCM
    //  qritemncm.AsString := ACBrNFe1.NotasFiscais.Items[0].NFe.Det.Items[x].Prod.NCM;

      //CEST
    // QRITEMcest.Asstring := ACBrNFe1.NotasFiscais.Items[0].NFe.Det.Items[x].Prod.CEST;
     ...
     // CST
      ...
      if vPrecoVenda > 0 then
        ...
        qritemPRECOVENDA.ASFLOAT := qritemPRECOCUSTO.ASFLOAT / qritemFRACAO.ASFLOAT;
    ...
    begin
      ...
      begin
        qrpagar.Insert;
        qrpagarCODIGO.AsString := frmprincipal.codifica('000046','update');;
...
begin
  ...
  begin
    ...
    inicia: { : INICIA VERIFICAÇÃO SE O PRODUTO ESTÁ PRONTO PARA RECEBER MOVIMENTAÇÃO - HISTORICO DE COMPRAS }
      ...
      begin
        ...
        qrproduto_movCODIGO.AsString := frmprincipal.codifica('000032','update');
  ...
  // COPIA XML IMPORTADO PARA A PASTA REFERENTE AO MES !

  if not fileexists(Destino + ExtractFileName(origem)) then
       begin
        if not movefile(pchar(origem),pchar(destino +'\'+ ExtractFileName(origem))) then
          ShowMessage('Não foi possivel criar uma cópia do Xml !');
       end;
   Destino := '';
   origem := '';
...
procedure Tfrmimporta_nfe.FormClose(Sender: TObject; var Action: TCloseAction);
begin
Action := CAFREE;
end;
...
begin
  ...
  frmmodulo.qrfornecedorCODIGO.Value := frmprincipal.codifica('000009','update');
...
begin
  ...
  begin
    ...
    QryCadastraProdutoCODIGO.Value := frmprincipal.codifica('000025','update');
